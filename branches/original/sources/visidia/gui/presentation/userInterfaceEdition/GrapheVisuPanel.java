...
/**
 * Un GrapheVisuPanel est un JPanel sur lequel le graphe est affiche et
 * qui permet d'interagir avec le graphe.
 *
 * <BR> Pour bien comprendre la separation des taches entre les classes Editeur et
 * GrapheVisuPanel, on peut imaginer plusieurs objets GrapheVisuPanel associes
 * a une seule instance de Editeur pour editer plusieurs parties d'un meme graphe
 * en meme temps.
 **/
public class GrapheVisuPanel extends JPanel implements MouseListener, MouseMotionListener, KeyListener {
    ...
    /** L'editeur auquel est associe le GrapheVisuPanel */
    ...
    /** Ordonnee de l'ancienne position de la souris au debut d'un drag-and-drop **/
    protected int y_ancien;
    /** Objet situe a la position a laquelle est la souris **/
    ...
    /** Signifie si un drag-and-drop d'un sommet est en court, mais avec deplacement d'un objet deja existant **/
    private boolean drag_n_drop_sommet_existant;
    private int dx;
    private int dy;
    /** Signifie si un drag-and-drop d'une selection est en court **/
    ...
    /** Abscisse de la position initiale de la souris lors d'une selection **/
    private int x_origine;
    /** Ordonnee de la position initiale de la souris lors d'une selection **/
    private int y_origine;
    /** Signifie si une selection par un rectangle est en court **/
    ...
    /**
     * Instancie un GrapheVisuPanel associe a l'editeur passe en argument.
     **/
    ...
    /**
     * Methode appellee quand on appuie sur un bouton de la souris. Suivant ce que
     * l'on trouve sous la souris, on appelle les autres methodes de la classe.
     **/
	...
	if(drag_n_drop_sommet) {
	    try {
		/* Si on fusionne deux sommets, il faut retirer le sommet cree de la liste */ 
		/* des informations de undo, sinon, il sera recree lors d'un redo eventuel!*/
		SommetDessin sommet_en_dessous =
		    editeur.getVueGraphe().sommet_en_dessous(x, y,objet_sous_souris);
		
		//ww : On efface l'ancienne arete si OUI, puisque une autre arete a ete cree, mais si l'utilisateur boucle sur le meme sommet, ceci n'efface pas la pseudo-arete.
	    ...
	    try {
		/* On fusionne deux sommets existants par deplacement de l'un d'eux. */
		SommetDessin sommet_en_dessous =
		    editeur.getVueGraphe().sommet_en_dessous(x, y,objet_sous_souris);
	    ...
	    //selection des objets dans la zone rectangulaire : c'est un nouveau undoGroup.
    ...
    /**
     * L'appui du bouton du milieu de la souris permet de deplacer un objet,
     * la selection ou la zone visible du plan de travail.
     **/
  ...
  /**
   * L'appui du bouton gauche de la souris permet de creer un nouveau
   * sommet (avec arete si on clique sur un sommet deja existant).
   **/
  ...
  /** 
   * Permet de deplacer l'objet sous le curseur en meme temps que la
   * souris au cas ou on maintient le bouton du milieu appuye.
   **/
  ...
  /**
   * Permet de creer un sommet et une arete quand on maintient le bouton
   * gauche de la souris appuye. Si on relache la souris au dessus d'un sommet
   * existant, la methode <b>mouseRelease</b> fusionne ce sommet avec le nouveau
   * sommet cree.
   **/
  public void glisseBoutonGauche(int x, int y) {
    ...
    if(objet_sous_souris != null) {
	 ...
	 this.ancien_sommet_sous_souris =
	  ...
	  // On sauvegarde l'objet UndoObject contenant ce sommet : 
	  // on peut en effet avoir a le supprimer en cas de fusion avec un sommet existant.
	  ...
	  editeur.undoInfo.addInfo(new AjouteObjet(a));
          ...
          // Quand on drag un sommet, on veut un positionnement precis par
          // rapport au curseur, on sauvegarde donc la position relative
          // du sommet par rapport au curseur, ici elle est nulle.
          dx = dy = 0;
        }
      }
      this.scrollRectToVisible(new Rectangle(x-10,y-10,30,30));
  ...
  /**
   * Methode privee qui gere le deplacement d'un sommet avec
  ...
  private void glisseSommet(int x, int y) {
    ...
    // remarque: dans le cas d'un drag-n-drop, on n'utilise pas x_ancien et y_ancien, mais
    // dx et dy pour assurer un positionnement plus precis (eviter les derives entre la
    // position du curseur et celle de l'objet deplace), on actualise cependant les variables
    // x_ancien et y_ancien pour homogeneiser avec le cas general.
  }

  /**
   * Methode de dessin du carre de selection.
   **/
