...
/**
 * Un GrapheVisuPanel est un JPanel sur lequel le graphe est affiché et
 * qui permet d'interagir avec le graphe.
 *
 * <BR> Pour bien comprendre la séparation des tâches entre les classes Editeur et
 * GrapheVisuPanel, on peut imaginer plusieurs objets GrapheVisuPanel associés
 * à une seule instance de Editeur pour éditer plusieurs parties d'un même graphe
 * en même temps.
 **/
public class GrapheVisuPanel extends JPanel implements MouseListener, MouseMotionListener, KeyListener {
  ...
  /** Abscisse de l'ancienne position de la souris au début d'un drag-and-drop **/
  protected int x_ancien;
  /** Ordonnée de l'ancienne position de la souris au début d'un drag-and-drop **/
  ...
  /** Signifie si un drag-and-drop d'un sommet est en court, mais avec déplacement d'un objet déjà existant **/
  ...
  /** Ordonnée de la position initiale de la souris lors d'une sélection **/
  ...
  protected Vector vecteurImages;
    ...
    /**
     * Instancie un GrapheVisuPanel associé à l'éditeur passé en argument.
     **/
    ...
    /**
     * Méthode appellée quand on appuie sur un bouton de la souris. Suivant ce que
     * l'on trouve sous la souris, on appelle les autres méthodes de la classe.
     **/
    ...
    /**
     * Méthode appellée quand on relâche un bouton de la souris.
     **/
 public void mouseReleased(MouseEvent evt) {
	 ...
	 try {
	     /* Si on fusionne deux sommets, il faut retirer le sommet créé de la liste */ 
	     /* des informations de undo, sinon, il sera recréé lors d'un redo eventuel!*/
      ...
      //ww : On efface l'ancienne arete si OUI, puisque une autre arete a ete cree, mais si l'utilisateur boucle sur le meme sommet, ceci n'efface pas la pseudo-arete.
	   if (editeur.getVueGraphe().rechercherArete(((SommetDessin)ancien_sommet_sous_souris).getEtiquette(), ((SommetDessin)sommet_en_dessous).getEtiquette()) != null)
			 {
			     editeur.getVueGraphe().delObject(
(AreteDessin)((editeur.getVueGraphe()).rechercherArete(
  ((SommetDessin)ancien_sommet_sous_souris).getEtiquette(),
  ((SommetDessin)sommet_en_dessous).getEtiquette())
));
			 }
...
(AreteDessin)((editeur.getVueGraphe()).rechercherArete(
  ...
  ((SommetDessin)objet_sous_souris).getEtiquette())
	 ...
	 try {
	     /* On fusionne deux sommets existants par déplacement de l'un d'eux. */
	     SommetDessin sommet_en_dessous =
		 editeur.getVueGraphe().sommet_en_dessous(x, y,
								 objet_sous_souris);
	     editeur.undoInfo.newGroup("Separate merged vertices", "Merge vertices");
	     
	     editeur.undoInfo.addInfo(new FusionneSommet((SommetDessin)objet_sous_souris,
							   sommet_en_dessous,
							   drag_n_dropVertex_X,
							   drag_n_dropVertex_Y));								
   ...
   * sommet (avec arête si on clique sur un sommet déjà existant).
   **/
  public void appuiBoutonGauche(int x, int y) {
    ...
    try {
      ...
      //Création d'un sommet : nouvel undoGroup.
   ...
   * souris au cas où on maintient le bouton du milieu appuyé.
   **/
  ...
  /**
   * Permet de créer un rectangle de sélection au cas ou on maintient
   ...
   * gauche de la souris appuyé. Si on relâche la souris au dessus d'un sommet
   * existant, la méthode <b>mouseRelease</b> fusionne ce sommet avec le nouveau
   * sommet créé.
   **/
