...
/** Cette classe implémente la structure de données utilisée pour gérer les opérations de undo/redo. **/

public class UndoInfo extends Vector {
  
  /** L'index de l'opération courante. */
  ...
  /** Ajoute une opération dans la liste, et incrémente le nombre d'opérations simples dans l'opération complexe courante. */
  public void addInfo(UndoObject objet) {
    currentGroup().add(objet);
  }

  /** Detruit les informations concernant les groupes créés postérieurement au groupe courant. */  
  private void trimGroups() {
    removeRange(curseur() + 1, size());
  }    
  
  /** Cette méthode permet d'annuler l'opération complexe courante, en annulant chacune des opérations simples qui la composent. */
  public void undo() {
    currentGroup().undo();
    curseur--;
  }
  
  /** Cette méthode permet de restaurer l'opération complexe courante, en restaurant chacune des opérations simples qui la composent. */
  public void redo() {
    curseur++;
    currentGroup().redo();
  }
    
  /** Retourne VRAI si il reste au moins une opération complexe susceptible d'être annulée. */
  public boolean undoMore() {
    return (curseur >= 0);
  }

  /** Retourne VRAI si il reste au moins une opération complexe susceptible d'être restaurée. */
  ...
  /**  Retire l'UndoObject qui contient la FormeDessin passée en argument,  dans le groupe courant */
  ...
  /** Cette méthode permet d'annuler les "i" dernières opérations (ou toutes les opérations si il y en a moins de "i") */
  ...
  /** Cette méthode permet de restaurer les "i" dernières opérations (ou toutes les opérations si il y en a moins de "i")*/
