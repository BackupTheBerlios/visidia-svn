...
public class Simulator {
    ...
    /**
     * crée une instance de simulateur à partir du graphe "graph", 
     * et des files de messages "inQueue" et "outQueue" qui permettent de  
     * communiquer avec l interface graphique
     */
    /*
    */
    public Simulator( SimpleGraph netGraph, Queue evtQueue, Queue ackQueue, AlgoChoiceInterface choice){
	...
	while(vertices.hasMoreElements() && netVertices.hasMoreElements()){
	    ...
	    processData.msgQueue = new Queue();
    ...
    public Simulator(SimpleGraph netGraph, Queue evtQueue, Queue ackQueue){
	graph = (SimpleGraph) netGraph.clone();
	evtObjectTmp = new Hashtable();
	evtQ = evtQueue;
	ackQ = ackQueue;
     ...
     */
    void sendTo(Integer senderId, int door, Message msg)throws InterruptedException{	
	...
	//System.out.println("receiver= "+(getDraw(senderId.intValue()) || getDraw(receiverId.intValue())));

	msg.setVisualization((getDraw(senderId.intValue())||getDraw(receiverId.intValue())));
	//System.out.println("receiver= "+msg.getVisualization());
	//MessagePacket msgPacket = new MessagePacket(senderId, door, receiverId, receiveDoor, (Message)msg.clone());
	
	MessagePacket msgPacket = new MessagePacket(senderId, door, receiverId, receiveDoor, msg);
	pushMessageSendingEvent(msgPacket); 
	//	((ProcessData)receiverVertex.getData()).msgQueue.put(msgPacket);
    }

    void sendToNext(Integer senderId, Message msg) throws InterruptedException{
	...
	if(receiverId != null) 
		...
		sendTo(senderId, senderDoor, msg);
	    }
    }
	...
	if(c != null){
	    msgPacket = (MessagePacket)procData.msgQueue.get(c);
	}
	else{
	    msgPacket = (MessagePacket)procData.msgQueue.get();
	}
    ...
    /**
     * retourne le degré du noeud <i>id</id> .
     */
    ...
    /**
     * instancie les thread à partir du graphe et de l'algorithme donné 
     *en paramètre et lance l'exécution. Pour le bon fonctionnement du systeme
    ...
    */
	...
	public void handleMessageSentAck(MessageSendingAck msa)throws InterruptedException{
	    MessagePacket msgPacket = (MessagePacket) evtObjectTmp.remove(msa.number());
	    procs[msgPacket.receiver().intValue()].msgQueue.put(msgPacket);
	    numGen.free(msa.number().longValue());
	}
    }
