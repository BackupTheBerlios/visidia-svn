...
\subsection{Vue globale d'un algorithme à base d'agents mobiles dans \visidia}
...
\texttt{SynchronizedAgent}, dans le cas où l'algorithme est synchrone.

Dans les deux cas, il vous faudra implémenter la méthode \texttt{init()}.
...
La méthode \texttt{init()} que vous implémenterez ressemblera en première
approche à ceci ~:\\
\begin{verbatim}
public void init ()
{
...
but de les réutiliser dans la phase de calcul car ces variables n'apparaîtront
pas dans le \emph{WhiteBoard} de l'agent qui est sensé représenter la mémoire de
ce dernier et vous perdrez par la même occasion certaines possibilités de
\visidia.

La phase de calcul devrait pouvoir être placée dans une fonction à part, cela
évite entre autre les pseudo-variables globales (celles qui ne sont pas dans le
...
êtes arreté ainsi que des variables locales que vous avez déclarées en passant
...
\subsection{Description de l'API utile pour l'implémentation des
  algorithmes}
\subsubsection{Déplacement de l'agent}
...
paramètre --- soit sous forme de chaîne de caractères soit en donnant
...
déplacement choisit lui-même le port.
...
vous faudra passer par la méthode \texttt{moveToDoor()} et lui donner le
...
\texttt{getArity()}), celui-ci pouvant être nul et les ports pouvant
être fermés il ne faudra pas oublier de 
prévoir ces cas.
...
port d'arrivée de l'agent.

Pour renvoyer l'agent d'où il est venu, vous pouvez
utilisez la méthode \texttt{moveBack()}.

\subsubsection{Modification et accès à la mémoire de l'agent}
...
\fg\  de l'algorithme. La seconde permet quant-à elle d'accéder à une
de ces variables.

Les variables des agents sont stockées sous la forme
(identifiantDeVariable, Valeur). Ceci permet d'avoir une grande
liberté dans le choix de la valeur qui peut être une structure
complexe (aussi complexe qu'une classe \emph{Java} peut l'être).
...
\subsubsection{Modification de l'état des ports}
...
plupart des cas, les états prédéfinis devraient vous suffire. Ceux-ci sont au nombre
...
\subsubsection{Modification et accès à la mémoire d'un sommet}

La modification de la mémoire d'un sommet est un peu plus délicate que la
modification de la mémoire d'un agent puisque dans le cas du sommet il faut gérer
l'accès concurrent à la ressource. Pour cela, vous avez la possibilité de
verrouiller la mémoire d'un sommet grâce à la méthode
...
Si le sommet est déjà verrouillé la méthode \texttt{lockVertexProperties()}
...
sommet se fait de la même manière que pour la mémoire d'un agent grâce aux
méthodes \texttt{getVertexProperty()}, \texttt{setVertexProperty()} et
\texttt{getVertexPropertyKeys()}.

\subsubsection{Création et clone d'agent}
\visidia vous permet de créer des agents lors de l'exécution de vos algorithmes.
Pour cela, vous
pouvez utilisez la méthode \texttt{createAgent()}, en lui fournissant en
paramètre la classe de l'agent à créer, ou si vous souhaitez envoyer cet agent
sur un port particulier la méthode
\texttt{createAgentAndSend()},en lui donnant la  classe de l'agent à créer et le
port sur lequel l'agent créé devra être envoyé.
...
\subsubsection{Récupérations des informations du simulateur}
...
exemple son identité (\texttt{getIdentity()}, sa classe (\texttt{className()})
et son nom (\texttt{toString()}).

\subsubsection{Les statistiques propres à l'algorithme}
...
\subsubsection{Autres méthodes intéressantes pour l'implémentation des agents}
...
votre algorithme peut avoir des résultats surprenants.
