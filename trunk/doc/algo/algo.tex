...
les arêtes que sur les ports eux-même --- peuvent avoir un état. Cet état
...
l'accès concurrent à la resource. Pour cela vous avez la possibilité de
...
attend que le sommet soit déverroullé pour le vérouiller et rendre la main à
l'agent. Cela pouvant être assez contraignant vouz pouvez vérifier si le sommet
est déjà verrouillé avant de le verrouiller grâce à la méthode
\texttt{vertexPropertiesLocked()} qui vous retournera un boolèen indiquant si le
...
paramétre la classe de l'agent à créer, et si vous souhaitez envoyer cet agent
sur un port particulié un port particulié la méthode
...
méthodes \texttt{cloneAgent()}, sans paramétre, et \texttt{cloneAndSend()}, en
...
de \texttt{AbstractStat} en paramétre. Il vous faudra créer aussi votre classe
