...
d'identifiant plus fort, ou également lorsqu'il rencontre un agent \textit{Spanning_Tree_Agent}
...
\textit{Spanning_Tree_Agent} passe par sa racine, il créait un agent \textit{Depth_Traversal_Agent}
et lui indique l'identifiant de l'arbre qu'il doit parcourir. L'agent
\textit{Spanning_Tree_Agent} se veille dans le sommet en attendant le résultat
de l'agent \textit{Depth_Traversal_Agent}. Lorsque ce dernier à fini (il se trouve donc sur la racine avec l'autre
...
couvrant et l'agent \textit{Spanning_Tree_Agent} se termine à son tour.
 

\item Pour annexer un sous arbre, nous avons implémenté un agent
\textit{Annexing_SubTree_Agent}, aussi appelé $clone$.
Ce dernier est créé par un agent \textit{Spanning_Tree_Agent} afin d'annexer un
...
l'aide des tracer laissées par l'agent \textit{Spanning_Tree_Agent} ayant créé
