...
Un réseau est un graphe dont les sommets sont les processurs et les arétes sont les liens de communication.\\ 

\subsection*{Notions d'algorithmique distribuée}

Il existe de nombreux modèles permettant de décrire des algorithmes distribués. Parmi eux, deux ont été développés dans \visidia : la communication par
échange de message et par agent.\\

Dans la suite, on parlera de mémoire d'un agent et d'un sommet mais la mémoire d'un sommet contient la whiteboard et l'etat de ses ports.\\

\subsubsection*{Communication par message}

Dans ce modèle, chaque noeud représente un système ou un processeur. Sur chaque
noeud est exécuté un algorithme disposant de certaines primitives qui lui permettent de
communiquer à l'aide de messages avec ses noeuds voisins.
Les noeuds sont reliés entre eux par des canaux de communication (les arêtes du graphe) par lesquels transitent les messages.
...
L'application \visidia a été réalisée en respectant une séparation en trois parties, interface graphique, simultateur et algorithmes.
L'interface graphique permet à l'utilisateur d'interagir avec le programme et deconsulter l'avancement d'une exécution d'agent sur un graphe. Elle est synchronisée avec le simultateur pour éviter toute incohérence. Elle fournit quelques statistiques de base sur l'exécution. Le simultateur coordonne les opérations. C'est le coeur de \visidia. Enfin, ce sont les agents  qui vont réaliser ces opérations sur le graphe.\\

Une bibliothèque  de primitives de  base (API) est mise à disposition de l'utilisateur afin qu'il puisse réaliser ses propres algorithmes à base d'agent.\\
...
pas aisée et la procédure est peu documentée, le client a donc besoin que les algorithmes soient implémentés pour lui.\\
...
l'altération des mémoires d'un noeud du réseau ou d'un agent, voire du crash
