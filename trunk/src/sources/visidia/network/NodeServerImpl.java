...
/**
 * This class represents a Local Node. when distributing the simulation graph
 * nodes over the network graph, we form local graphs for each one of network
 * graph node. to cretae this local graphs we use the local node. This is a
 * local node of type "NodeServer". see http://www.labri.fr/~derbel/these for
 * more details to sum, it creates the nodes that were assigned to this
 * LocalNode.
 * 
 * @author DERBEL bilel
 * @version 1.0
...
public class NodeServerImpl extends UnicastRemoteObject implements NodeServer {
	...
	// Remote Method to Initilise the nodes on the the <code>serverName</code>
	// host
	// public Hashtable initialiser(Hashtable liste, SimpleGraph graph, String
	...
	public Hashtable initialize(Vector vect, String nom, String visuUrl)
		...
		for (int i = 0; i < vect.size(); i++) {
			try {
				...
				NodeInterfaceTry nodeSimul = new NodeTry(node, nom, visuUrl,
						this.registryPort);
	...
	public void reInitialiser() throws RemoteException {
		try {
			...
			while (theNodes.hasMoreElements()) {
				...
				NodeInterfaceTry nodeSimul = (NodeInterfaceTry) this.graphStub
						.get(node);
	...
	public void register(String visuHost, String url) throws RemoteException {
		try {
			RegistrationThread rt = new RegistrationThread(visuHost, url,
