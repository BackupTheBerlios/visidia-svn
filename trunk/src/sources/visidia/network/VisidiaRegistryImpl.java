...
public class VisidiaRegistryImpl extends UnicastRemoteObject implements VisidiaRegistry {
	...
	LocalNodeSelection lns = new LocalNodeSelection("Local Node Selection",sizeOfTheGraph,this.parent,this);
	this.refresh(lns);
	lns.setVisible(true);
    }
	...
	try {
	    ...
	    JOptionPane.showMessageDialog(this.parent,"The Visidia Registration is not running : \n"+e.toString(),"Error",JOptionPane.WARNING_MESSAGE);
	    System.out.println("##########################################################");
...
class LocalNodeSelection extends JFrame implements ActionListener {
	...
	try{
	    this.visuHost = new JTextField(java.net.InetAddress.getLocalHost().getHostName());
	} catch (Exception e) {
	    e.printStackTrace();
	}
...
class TableModel extends AbstractTableModel {
	...
	Vector v = new Vector();
	for (int i=0; i<this.data.size(); i++){
	    if (((Boolean)((Vector)this.data.elementAt(i)).elementAt(2)).equals(new Boolean(true))){
		v.addElement(new Integer(i));
	    }
	}
	return v;
