...
/**
 * This is one implementation of the Simultor_Rmi_Int interface. It allows the
 * initialization of nodes and communicates to the GUI the happening events in
 * the network (e.g : node state changes).
 */
public class Simulator_Rmi extends UnicastRemoteObject implements
		Simulator_Rmi_Int {
	...
	/**
	 * The name in the RMI/registry of the console. It is important to give not
	 * already used name when running two simulation for example.
	 */
	...
	// tampons d'identification des acquittement des evenement suivant le noeud
	// source
	 ...
	 */
	public Simulator_Rmi(SimpleGraph netGraph, VQueue evtVQueue,
			VQueue ackVQueue, String name, String simUrl, String regPort)
	 ...
	 * "NodeServer" or "NodeFactory". So, we must specify which type we want to
	 * use in the <i> tag </tag> parameter.
	 */
	public void initializeNodes(LocalNodeTable networkParam)
			throws RemoteException {
		...
		while (v_enum.hasMoreElements()) {
			...
			while (e.hasMoreElements()) {
				...
				try {
					NodeServer nodeServer = (NodeServer) Naming.lookup("rmi://"
							+ host + ":" + this.registryPort + "/NodeServer/"
							+ localNode);
					this.add(nodeServer.initialize(vect, this.simulatorHost,
							this.simulatorUrl));
					Vector v = new Vector();
					v.addElement(host);
					...
					System.out
							.println("ERREUR : dans l'initialisation des noeuds");
	...
	/**
	 * using the table graphStub, the console contact the nodes and order them
	 * to begin running the algorithm
	 */
	public void startServer(AlgorithmDist algo) throws RemoteException {
		/**
		 * Initialisation des noeuds distant : Pour chaque noued --> creation
		 * d'une table (key = port, value = neighbor identity)
		 */
		...
		Date date1 = new Date();
		System.out
				.println("Calcul de la table des voisins et transmission des parametres :");
		while (theNodes.hasMoreElements()) {
			try {
				String nodeLocation = (String) theNodes.nextElement();
				NodeInterfaceTry nodeServer = (NodeInterfaceTry) this.graphStub
						.get(nodeLocation);
				SimpleGraphVertex neighbors = (SimpleGraphVertex) this.graph
						.vertex(new Integer(nodeLocation));
				...
				while (keys.hasMoreElements()) {
					...
					NodeInterfaceTry nit = (NodeInterfaceTry) this.graphStub
							.get(aNode.toString());
				...
				System.out
						.println("Erreur 1 dans startServer-->Simulator_Rmi :"
								+ e);
				e.printStackTrace();
			}
		}
		...
		while (theNodesBis.hasMoreElements()) {
			try {
				...
				NodeInterfaceTry nodeServerBis = (NodeInterfaceTry) this.graphStub
						.get(nodeLocationBis);
				nodeServerBis.startRunning();
			} catch (Exception e) {
				System.out
						.println("Erreur 2 dans startServer-->Simulator_Rmi :"
								+ e);
			}
		}
		Date date3 = new Date();
		System.out.println("Demarrage de l initialisation startServer() : "
				+ date1.toString() + " et fin : " + date2.toString());
		System.out.println("Demarrage des noeuds : " + date2.toString()
				+ " et fin : " + date3.toString());
	...
	public void abortSimulation() {
		...
		try {
			...
			while (theServers.hasMoreElements()) {
				...
				NodeServer nodeServer = (NodeServer) this.nodeServerStub
						.get(server);
	...
	/**
	 * count the number of nodes which algorithm has terminated. if all the node
	 * do nothing (the distributed algorithm is terminated, it send an
	 * AlgorithmEndEvent to the GUI
	 */
		...
		synchronized (this.terminatedThreadCountSynchro) {
			...
			if (this.terminatedThreadCount == this.graph.size()) {
				try {
					...
					// NodeInterfaceTry nodeServer =
					// (NodeInterfaceTry)graphStub.get(nodeLocation);
					// nodeServer.getRound
					System.out.println("Le nombre final des messages est de : "
							+ this.getMessageNumber());
					Date dateFin = new Date();
					System.out.println("Algorithme termine a : "
							+ dateFin.toString());
	...
	/**
	 * contact all the remote nodes and order them to wedge the simulation. This
	 * happens when clicking on the "pause" button.
	 */
	public void wedge() throws RemoteException {
		try {
			...
			System.out.println("Erreur : tentative d interruption des threads"
					+ e);
		}
	...
	public void unWedge() throws RemoteException {
		try {
			...
			System.out
					.println("Erreur : tentative de relance des threads " + e);
		}
	...
	public void setMessageType(MessageType msgType, boolean state)
		...
		try {
			...
			System.out
					.println("Erreur : la tentative de changment du type de messages a afficher a echoue "
							+ e);
			e.printStackTrace();
		}
	}
	...
	public int getMessageNumber() throws RemoteException {
		...
		try {
			...
			while (theNodes.hasMoreElements()) {
				...
				totalMessageNumber = totalMessageNumber
						+ ((Integer) vect.elementAt(0)).intValue();
				totalSynchMessage = totalSynchMessage
						+ ((Integer) vect.elementAt(2)).intValue();
				totalLabel = totalLabel
						+ ((Integer) vect.elementAt(1)).intValue();
				totalSynch = totalSynch
						+ ((Integer) vect.elementAt(3)).intValue();
			}
		} catch (Exception e) {
			System.out
					.println("Erreur : la tentative de changment du type de messages a afficher a echoue "
		...
		System.out.println("Le nombre total de Synchronisation est de : "
				+ totalSynch);
		System.out.println("Le nombre total de label est de : " + totalLabel);
		System.out
				.println("Le nombre total de message de synchronisation est de : "
						+ totalSynchMessage);
		return totalMessageNumber;
	}

	/**
	 * creates a new NodePropertyChangeEvent message and put it in the event
	 * queue
	 */
	public void pushNodePropertyChangeEvent(Integer nodeId, Object key,
			Object value) throws InterruptedException, RemoteException {
		try {
			Long num = new Long(this.numGen.alloc());
			this.evtObjectTmp.put(num, nodeId);
			NodePropertyChangeEvent npce = new NodePropertyChangeEvent(num,
					nodeId, key, value);
			this.evtQ.put(npce);
		} catch (Exception e) {
	...
	/**
	 * creates a new EdgeStateChangeEvent message and put it in the event queue
	 */
	public void pushEdgeStateChangeEvent(Integer nodeId1, Integer nodeId2,
			EdgeState es) throws InterruptedException, RemoteException {
		try {
	...
	/**
	 * creates a new MessageSendingEvent message and put it in the event queue
	 */
	public void pushMessageSendingEvent(Integer senderId, int door,
			Integer receiverId, Message msg) throws InterruptedException,
			RemoteException {
		try {
			...
			MessagePacket mesgPacket = new MessagePacket(senderId, door,
	 ...
	 */
	public void register(NodeServerImpl nsi, String host, String url)
			throws RemoteException {
		System.out.println("Local Node (" + host + "," + url
				+ ") try to register him self on the console");
	}
	...
	class AckFreeHandler extends Thread {
		...
		public void handleNodePropertyChangeAck(SimulAck sa)
			...
			try {
				Integer nodeSource = (Integer) Simulator_Rmi.this.evtObjectTmp
						.remove(sa.number());
				NodeInterfaceTry node = (NodeInterfaceTry) this.locReso
						.get(nodeSource.toString());
				node.free();
