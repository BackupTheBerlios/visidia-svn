...
/**
 * this class allows to handle automatically a Lamport clock. Each time a node
 * sends a message or a node changes its properties the lamport clock is
 * increased by one. When receiving a message the lamort clock is set to be the
 * (maximum of the clock value of the received message and the clock of the
 * current node) + 1 (the one who is receiving). Note that, this can be handled
 * by hand using the setClock and getClock of the Message class. the method
 * description can be found in the Algorithm class.
 */

public abstract class LamportAlgorithm extends Algorithm implements Runnable,
		Cloneable {
	int lamportClock = 0;
	...
	/**
	 * instructions to be executed by the algorithm. The user must implement
	 * this method to run his algorithm
	 */
