...
/**
 * Attention : classe centrale. Elle représente l'entité principale qui permet
 * l'envoi et la réception de messages. Toutes les communications des threads
 * simulants les noeuds sont prises en compte ici.
 * 
 */
...
public class Simulator {
	...
	/*
	 * tampon de messages en transit sur le reseaux i.e qui ne sont pas encore
	 * mise dans la file de messages de leur destinataire.
	 */
	...
	/**
	 * crée une instance de simulateur à partir du graphe "graph", et des files
	 * de messages "inVQueue" et "outVQueue" qui permettent de communiquer avec
	 * l'interface graphique
	 */
	public Simulator(SimpleGraph netGraph, VQueue evtVQueue, VQueue ackVQueue,
			AlgoChoiceInterface choice) {
		...
		while (vertices.hasMoreElements() && netVertices.hasMoreElements()) {
			...
			processData.algo = choice
					.getAlgorithm(vertex.identity().intValue());
	...
	/**
	 * compte le nombre de fois qu'un acquitement de visualation relatif aux
	 * pulses est arrivé
	 */
	...
	/**
	 * acquitement des traitements de visualizations et surtout d'envoi de
	 * messages. On compte deux fois : un pour le siulEventHandler et deux pour
	 * le SimulationPanel
	 * 
	 */
	...
	public void nextPulse() {
		try {
			synchronized (this.lockSync) {
				this.countNextPulse = (this.countNextPulse + 1)
						% (this.graph.size() - this.terminatedThreadCount);
	...
	/**
	 * met le message dans la file d'attente du voisin de "srcId" correspondant
	 * à la porte "door".
	 */
	boolean sendTo(Integer senderId, int door, Message msg)
			throws InterruptedException {

		Integer receiverId = this.graph.vertex(senderId).neighbour(door)
				.identity();

		Vertex receiverVertex = this.graph.vertex(receiverId);
		int receiveDoor = receiverVertex.indexOf(senderId);

		MessagePacket msgPacket = new MessagePacket(senderId, door, receiverId,
				receiveDoor, msg);

		msg.setVisualization((this.getDraw(senderId.intValue()) || this
	...
	boolean sendToNext(Integer senderId, Message msg)
			throws InterruptedException {
	...
	Message getNextMessage(Integer nodeId, Door door, Criterion c)
			throws InterruptedException {
	...
	/**
	 * should work with any well deifined criterion. For the moment it works
	 * with the DoorPulseCriterion. return the first message received by nodeId
	 * which matchs the criterion dpc. If none, then return null.
	 * 
	 */
	Message getNextMessage(Integer nodeId, DoorPulseCriterion dpc)
			throws InterruptedException {
	...
	/**
	 * return a vector of all message received by nodeId and matching the
	 * criterion dpc. If none then return null. Not used for the moment don't
	 * work
	 */
	Vector getAllNextMessages(Integer nodeId, DoorPulseCriterion dpc)
			throws InterruptedException {
		...
		if (msgPacketVect != null) {
			Vector v = new Vector();
			dpc.setDoor(((MessagePacket) msgPacketVect.elementAt(0))
					.receiverDoor());
			dpc.setPulse(((MessagePacket) msgPacketVect.elementAt(0)).message()
					.getMsgClock());

			for (int i = 0; i < msgPacketVect.size(); i++) {
				v.addElement(((MessagePacket) msgPacketVect.elementAt(0))
						.message());
			}
			return v;
		}
	...
	boolean emptyVQueue(Integer nodeId, Criterion c)
			throws InterruptedException {
		ProcessData procData = this.procs[nodeId.intValue()];
	...
	Message getNextMessageFromPrevious(Integer nodeId, Criterion c)
			throws InterruptedException {
	...
	private void pushNodePropertyChangeEvent(Integer nodeId, Object key,
			Object value) throws InterruptedException {
		Long num = new Long(this.numGen.alloc());
		Object lock = new Object();
		this.evtObjectTmp.put(num, lock);
		NodePropertyChangeEvent npce = new NodePropertyChangeEvent(num, nodeId,
				key, value);
		synchronized (lock) {
			this.evtQ.put(npce);
			lock.wait();
	...
	private void pushEdgeStateChangeEvent(Integer nodeId1, Integer nodeId2,
			EdgeState es) throws InterruptedException {
		Long key = new Long(this.numGen.alloc());
		Object lock = new Object();
		this.evtObjectTmp.put(key, lock);
		EdgeStateChangeEvent esce = new EdgeStateChangeEvent(key, nodeId1,
				nodeId2, es);
		synchronized (lock) {
			this.evtQ.put(esce);
			lock.wait();
	 ...
	 */
	private void pushEdgeColorChangeEvent(Integer nodeId1, Integer nodeId2,
			EdgeColor ec) throws InterruptedException {
		Long key = new Long(this.numGen.alloc());
		Object lock = new Object();
		this.evtObjectTmp.put(key, lock);
		EdgeColorChangeEvent ecce = new EdgeColorChangeEvent(key, nodeId1,
				nodeId2, ec);
		synchronized (lock) {
			this.evtQ.put(ecce);
			lock.wait();
	...
	private void pushMessageSendingEvent(MessagePacket mesgPacket)
		...
		MessageSendingEvent mse = new MessageSendingEvent(key, mesgPacket
				.message(), mesgPacket.sender(), mesgPacket.receiver());
		this.evtQ.put(mse);
	...
	/**
	 */
	public void changeEdgeState(Integer id, int door, EdgeState newEdgeState)
			throws InterruptedException {
	...
	/**
	 * Method which get the identificator of a vertex and calls the
	 * <i>pushEdgeColorChangeEvent</i> method
	 * 
	 * @param id
	 ...
	 * @throws java.lang.InterruptedException
	 */
	public void changeEdgeColor(Integer id, int door, EdgeColor newEdgeColor)
			throws InterruptedException {
		Integer neighbId = this.graph.vertex(id).neighbour(door).identity();
	...
	/**
	 * instancie les thread à partir du graphe et de l'algorithme donne en
	 * parametre et lance l'execution. Pour le bon fonctionnement du système les
	 * deux files d'attentes passées en parametre doivent être vide, mais ceci
	...
	public void startSimulation() {
		...
		/*
		 * while(threadGroup.activeCount() < graphSize){ try{
		 * Thread.currentThread().sleep(10); } catch(InterruptedException e){
		 * //e.printStackTrace(); } }
		 */
		this.started = true;
		System.out
				.println("thread count after start = " + Thread.activeCount());
	}

	/**
	 * This method attempts to stop all threads that are rinning in this
	 * simulator. Fistly the abstract method <code>abort()</code> is called on
	 * all Algorithm objects, and then all threads that run them are interrupted
	 * until there are no active thread on the graph.
	 */
	public void abortSimulation() {
		...
		/**
		 * ne fonctionne pas vraiment bien l'arrêt des thread, à revoir
		 */
		...
		// System.out.println("thread count after abort =
		// "+Thread.activeCount());
	}
	...
	/*
	 * public void setAlgoProperties(int id, Hashtable newProps){ Vertex vtx =
	 * graph.vertex(new Integer(id)); Hashtable props = procs[id].props;
	 * synchronized(props){ Enumeration v_enum = props.keys();
	 * while(v_enum.hasMoreElements()){ Object key = v_enum.nextElement();
	...
	void putNodeProperty(int id, Object key, Object value)
			throws InterruptedException {
		synchronized (this.procs[id].props) {
	...
	void terminatedAlgorithm() throws InterruptedException {
		...
		synchronized (this.lockSync) {
			// if (countNextPulse !=0) {
			int i = (this.countNextPulse + 1)
					% (this.graph.size() - this.terminatedThreadCount);

			if ((i == 0)
					&& (this.terminatedThreadCount != this.graph.size() - 1)) {
				try {
					this.pushNextPulseEvent();
				} catch (Exception e) {
					// aucune exception n'est normalement levée ici
	...
	class AckHandler extends Thread {
		...
		public void handleNodePropertyChangeAck(SimulAck sa)
				throws InterruptedException {
			Object lock = Simulator.this.evtObjectTmp.remove(sa.number());
			synchronized (lock) {
				/**
				 * normalement il n' y a qu'un seul thread (le noeud qui fait le
				 * changement) qui serait bloquée sur le lock.
				 */
				lock.notifyAll();
			}
		...
		public void handleEdgeStateChangeAck(SimulAck sa)
				throws InterruptedException {
			Object lock = Simulator.this.evtObjectTmp.remove(sa.number());
			synchronized (lock) {
				/**
		...
		public void handleEdgeColorChangeAck(SimulAck sa)
				throws InterruptedException {
			Object lock = Simulator.this.evtObjectTmp.remove(sa.number());
			synchronized (lock) {
				lock.notifyAll();
		...
		public void handleMessageSentAck_old(MessageSendingAck msa)
				throws InterruptedException {
			MessagePacket msgPacket = (MessagePacket) Simulator.this.evtObjectTmp
					.remove(msa.number());
			Vertex receiverVertex = Simulator.this.graph.vertex(msgPacket
					.receiver());
			((ProcessData) receiverVertex.getData()).msgVQueue.put(msgPacket);
			Simulator.this.numGen.free(msa.number().longValue());
		}

		public void handleMessageSentAck(MessageSendingAck msa)
				throws InterruptedException {
			MessagePacket msgPacket = (MessagePacket) Simulator.this.evtObjectTmp
					.remove(msa.number());
			Simulator.this.procs[msgPacket.receiver().intValue()].msgVQueue
					.put(msgPacket);
			Simulator.this.numGen.free(msa.number().longValue());
		}
