...
public abstract class Algorithm implements Runnable, Cloneable {
	...
	/**
	 * Retourne le premier message recu de la porte "door".Tant que le noeud ne
	 * recoit pas de message sur cette porte, il est bloque.
	 */
	protected Message receiveFrom(int door) {
		...
		Message msg = null;
		try {
			msg = this.sim.getNextMessage(this.nodeId, null, new DoorCriterion(
					door));
		} catch (InterruptedException e) {
			throw new SimulationAbortError();
		}
	 ...
	 * est vide,le noeud est endormi jusqu'a reception d'un message.
	 */
	...
	protected Message receive(Door door, MessageCriterion mc) {
		...
		Message msg = null;
		try {
			msg = this.sim.getNextMessage(this.nodeId, door,
	...
	protected final int previousDoor() {
		Integer previous = this.sim.getGraph().vertex(this.getId())
				.getPrevious();
	 ...
	 * 
	 * @param synob
	 *            the name of the SynObject.
	 * @param synal
	 *            the name of a synchronisation algorithm already defined. it
	 *            loads synObject and synal given in parametes and prepares
	 *            their use in the current algorithm.
	 */
	public final void setSynchronisation(String synal_name, String synob_name) {
		SynObject so;
		AbSynAlgo a;
		try {
			so = (SynObject) Class.forName(
					"visidia.simulation.synchro.synObj." + synob_name)
					.newInstance();
			a = (AbSynAlgo) Class.forName(
					"visidia.simulation.synchro.synAlgos." + synal_name)
					.newInstance();
			this.setSynob(so);
			this.setSynal(a);
			a.set(this);
			...
			System.out
					.println("Problem:Algorithm de synchronisation non trouve: "
							+ excpt);
		}
	...
	/**
	 * this method permites to an algorithm (especially synchronization) to copy
	 * the identity of the original algorithm.
	 */
	...
	/**
	 * Permet de relier le noeud au simulateur. "s" est defini comme simulateur
	 * du noeud.
	 */
	 ...
	 * methode s'il desire ecrire un algorithme sans passer par les regles de
	 * réécriture.
	 */
	public abstract void init();

	/**
	 * Methode faisant appel a <code>init()</code>. Cette methode est
	 * necessaire lors de l'instanciation du thread,un thread doit posseder la
	 * methode <code>run()</code>.
	 */
	...
	/**
	 * This method changes the color of an edge. Warning: You must include the
	 * java.awt.Color in your Algorithm
	 * 
	 * @param door
	 *            The number of the door
	 * @param color
	 *            A new color for the edge: <i>new ColorState(Color)</i>
	 */
	...
	/*
	 * // Permet de retourner le vertex voisin de la porte Vertex voisin =
	 * sim.getGraph().vertex(getId()).neighbour(door); // Retourne l'arete(de
	 * type Edge) entre la porte et le vertex voisin edge =
	 * voisin.edge(sim.getGraph().vertex(getId()).identity()); // Retourne
	 * l'objet representant l'arete edge.setColor(color);
	 */
	...
	/**
	 * Wakeup the thread if it was waiting to be connected to one neighbour at
	 * least. Can be called also if the vertex is not waiting.
	 */
	// PFA2003
	...
	/**
	 * Returns a string description of the algorithm Returns an empty string by
	 * default.
	 */
	// PFA2003
	...
	/**
	 * True if the thread is currently running. Returns true by default
	 */
	// PFA2003
