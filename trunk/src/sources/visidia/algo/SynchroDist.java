...
public class SynchroDist extends Algorithm {
	...
	public Collection getListTypes() {
		...
		typesList.add(SynchroDist.synchronization);
		typesList.add(SynchroDist.labels);
		typesList.add(SynchroDist.termination);
		return typesList;
	}
		...
		while (run) {
			...
			do {
				...
				if ((myL.compareTo(nodeA) == 0) && (!fir)) {
					if (father == -1) {
						this
								.sendAll(new StringMessage(myL,
										SynchroDist.labels));
					} else {
						for (int j = 0; j < this.getArity(); j++) {
							if (j != father) {
								this.sendTo(j, new StringMessage(myL,
										SynchroDist.labels));
							}
						}
					}
				...
				if (label[i].compareTo(nodeF) == 0) {
					...
					/*
					 * int choosenNumber =
					 * Math.abs(SynchronizedRandom.nextInt()); sendTo(i,new
					 * IntegerMessage(new Integer(choosenNumber),labels)); int
					 * res=(((IntegerMessage)
					 * receiveFrom(i)).data()).intValue();
					 * 
					 * if (choosenNumber>res) { myL=nodeA;
					 */
					neighbours[i] = false;
				...
				if ((myL.compareTo(nodeN) == 0) && (father != -1)) {
					...
					this.sendTo(father, new StringMessage(nodeL,
							SynchroDist.labels));
			...
			do {
				if ((myL.compareTo(nodeN) == 0) && (!fir) && (father != -1)) {
					/*
					 * int j=0; while (!neighbours[j]) j++; if (j<getArity()) {
					 * sendTo(j,new StringMessage(nodeF,labels));
					 */
					// if (father!=-1) {
					this.sendTo(father, new StringMessage(nodeF,
							SynchroDist.labels));
				...
				if (!fir) {
					/* String la= */((StringMessage) this.receive(door))
							.data();
					i = door.getNum();
