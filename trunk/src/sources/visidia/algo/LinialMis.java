...
public class LinialMis extends Algorithm {
	...
	public void init() {
		...
		/*
		 * initrally, the node is not in the MIS and it is not a neighbor of a
		 * node in the MIS
		 */
		...
		while (!covered) {
			...
			if (finishedNeighbors.size() != nodeDegree) {
				...
				for (Integer i = 0; i < nodeDegree; i++) {
					if (!finishedNeighbors.contains(i)) {
						/*
						 * receive a message from the neighbor connected on door
						 * i
						 */
				...
				if (biggest) {
					...
					/*
					 * set the label of the node to M in the local data
					 * structure of the node: the node is labeled M in the GUI
					 */
					...
					if (covered) {
						...
						/*
						 * inform active neighbors that the node is covered by
						 * the MIS
						 */
						for (Integer i = 0; i < nodeDegree; i++) {
							if (!finishedNeighbors.contains(i))
						...
						/*
						 * inform active neighbors that the node is not covered
						 * by the MIS
						 */
						for (Integer i = 0; i < nodeDegree; i++) {
							if (!finishedNeighbors.contains(i))
