...
/**
 * The algorithm of Ricart and Agrawala solves the problem of conflicts between
 * ressources. The conflict between a node and its neihbours happens when a node
 * changes its state to Hungry. The node which have the state Hungry may ask its
 * neighbours the ressource. Then, if its neighbour answer by an affirmitive, it
 * changes its state into Eating. In this version, we init the graph with the
 * state Thinking. Randomly, some states change their label to Hungry node and
 * try to access the ressource. If the ressource is available (all its
 * neighbours send him the autorization) then it changes its states to Eating
 * else it keeps its states to Hungry and increment his state. This version
 * increment the node state when it cannot access its critical section during
 * the synchronization.
 */
...
public class Ricart_Agrawala_LC1_V1 extends Algorithm {
	...
	public Collection getListTypes() {
		...
		typesList.add(Ricart_Agrawala_LC1_V1.synchronization);
		typesList.add(Ricart_Agrawala_LC1_V1.labels);
		typesList.add(Ricart_Agrawala_LC1_V1.booleen);
		return typesList;
	}
	...
	public void init() {
		...
		while (run) {
			...
			if (choosenNumber >= myNumber) {
				if (this.getProperty("label").toString().substring(0, 1)
						.compareTo("H") == 0) {
			...
			if (synchro != null) {
				if (((Integer) synchro.elementAt(0)).intValue() == -1) {
					...
					int myLabelInt = (new Integer(nodeLabel.data().substring(2)))
							.intValue();
					...
					for (int i = 0; i < arity; i++) {
						String msg = ((StringMessage) this.receiveFrom(i))
								.data();
						String neighLabelString = msg.substring(0, 1);
						int neighLabelInt = (new Integer(msg.substring(2)))
								.intValue();
	...
	public Vector starSynchro() {
		...
		this.sendAll(new IntegerMessage(new Integer(choosenNumber),
				Ricart_Agrawala_LC1_V1.synchronization));
