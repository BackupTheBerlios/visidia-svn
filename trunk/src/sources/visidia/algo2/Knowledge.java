...
public class Knowledge {
	...
	public void changeNeighbours(Vector newName) {
		...
		if (((Integer) newName.elementAt(0)).intValue() != ((Integer) newName
			...
			if (this.setKnowledge[0].isEmpty()) {
				...
				if (longKnow > 1) {
					if (((Integer) this.setKnowledge[0].elementAt(0))
						...
						// System.out.println("Longueur= "+longKnow+" ; vecteur
						// = "+setKnowledge[0]);
						...
						if (((Integer) this.setKnowledge[0]
							...
							for (int i = 0; i < longKnow - 1; i++) {
								if ((((Integer) this.setKnowledge[0]
										...
										.elementAt(0)).intValue())
										&& (((Integer) this.setKnowledge[0]
												.elementAt(i + 1)).intValue() <= ((Integer) newName
												.elementAt(0)).intValue())) {
									this.setKnowledge[0].insertElementAt(
	...
	public boolean endKnowledge(int graphS) {
		...
		for (int i = 1; i <= graphS; i++) {
			if (this.setKnowledge[i] == null) {
				var = false;
				break;
			} else if (this.setKnowledge[i].contains(new Integer(0))) {
				var = false;
				break;
			}
		}
	...
	/*
	 * public void afficherKnowledge(int graphS) { String mess="";
	 * 
	 * for (int i=1;i<=graphS;i++) mess=mess+"["+i+"]="+neighbourNode(i)+"\n";
	 * 
	 * javax.swing.JOptionPane.showMessageDialog(null, mess,
	 * "Knowledge",INFORMATION_MESSAGE ); }
	 */
}
