...
/**
 * This class is responsible of dealing with the events coming from the
 * simulator
 */
...
public class SimulEventHandler extends Thread {
	...
	/*
	 * events file from the simulator that will be traduce to the graphics
	 * interface by a state change or a message sent
	 */
	...
	public SimulEventHandler(FenetreDeSimulation f, VQueue evtPipe,
			VQueue ackPipe) {
	...
	public SimulEventHandler(AgentsSimulationWindow w, VQueue evtPipe,
			VQueue ackPipe) {
		this.evtPipe = evtPipe;
		this.ackPipe = ackPipe;
	...
	public SimulEventHandler(FenetreDeSimulationDist f, VQueue evtPipe,
			VQueue ackPipe) {
		this.evtPipe = evtPipe;
		this.ackPipe = ackPipe;
	...
	public void handleEdgeStateChangeEvt(SimulEvent se)
		...
		if (this.fenetreSimulDist == null) {
			if (esce.state() instanceof MarkedState) {
				...
				((this.fenetreSimul.getVueGraphe()).rechercherArete(esce
						.nodeId1().toString(), esce.nodeId2().toString()))
						.setEtat(state.isMarked());

			} else if (esce.state() instanceof SyncState) {
				SyncState state = (SyncState) esce.state();
				((this.fenetreSimul.getVueGraphe()).rechercherArete(esce
						.nodeId1().toString(), esce.nodeId2().toString()))
						.enluminerBis(state.isSynchronized());
				((this.fenetreSimul.getVueGraphe()).rechercherSommet(esce
						.nodeId1().toString())).enluminerBis(state
						.isSynchronized());
				((this.fenetreSimul.getVueGraphe()).rechercherSommet(esce
						.nodeId2().toString())).enluminerBis(state
						.isSynchronized());
			...
			if (esce.state() instanceof MarkedState) {
				MarkedState state = (MarkedState) esce.state();
				((this.fenetreSimulDist.getVueGraphe()).rechercherArete(esce
						.nodeId1().toString(), esce.nodeId2().toString()))
						.setEtat(state.isMarked());
				...
				((this.fenetreSimulDist.getVueGraphe()).rechercherArete(esce
						.nodeId1().toString(), esce.nodeId2().toString()))
						.enluminerBis(state.isSynchronized());
				((this.fenetreSimulDist.getVueGraphe()).rechercherSommet(esce
						.nodeId1().toString())).enluminerBis(state
						.isSynchronized());
				((this.fenetreSimulDist.getVueGraphe()).rechercherSommet(esce
						.nodeId2().toString())).enluminerBis(state
	...
	public void handleNodePropertyChangeEvt(SimulEvent se)
			throws InterruptedException {
		NodePropertyChangeEvent npce = (NodePropertyChangeEvent) se;
		if (this.fenetreSimulDist == null) {
			Hashtable tableSommet = ((this.fenetreSimul.getVueGraphe())
					.rechercherSommet(npce.nodeId().toString()))
					.getStateTable();
			tableSommet.put(npce.getKey(), npce.getValue());

			NodePropertyChangeAck npca = new NodePropertyChangeAck(npce
					.eventNumber());
			this.ackPipe.put(npca);
		} else {
			Hashtable tableSommet = ((this.fenetreSimulDist.getVueGraphe())
					.rechercherSommet(npce.nodeId().toString()))
					.getStateTable();
			tableSommet.put(npce.getKey(), npce.getValue());
	...
	public void handleAlgorithmEndEvent(SimulEvent se)
		...
		if (this.fenetreSimulDist == null) {
			...
			JOptionPane.showMessageDialog(this.fenetreSimul,
					"Algorithms are terminated");
			// fenetreSimul.simulationPanel().pause();
			// fenetreSimul.but_pause();

			AlgorithmEndAck aea = new AlgorithmEndAck(aee.eventNumber());
			// System.out.println("Threads have terminated: waiting for GUI
			// termination.");
			...
			JOptionPane.showMessageDialog(this.fenetreSimulDist,
					"Algorithms are terminated");
			throw new InterruptedException();
		}
	}

	private void handleNextPulseEvent(SimulEvent se)
			throws InterruptedException {
		NextPulseEvent npe = (NextPulseEvent) se;
		if (this.fenetreSimulDist == null) {
			this.fenetreSimul.setUpTimeUnits(npe.pulse());
			// se bloque jusqu'a la fin de la visualisation et donc de
