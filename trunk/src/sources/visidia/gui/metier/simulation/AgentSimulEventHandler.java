...
public class AgentSimulEventHandler extends Thread {
	...
	public void handleEdgeStateChangeEvt(SimulEvent se)
		...
		if (event.state() instanceof MarkedState) {
			...
			AreteDessin arete;
			arete = vue.rechercherArete(event.nodeId1().toString(), event
					.nodeId2().toString());
			...
			VueGraphe vue = this.agentsSimulationWindow.getVueGraphe();
			(vue.rechercherArete(event.nodeId1().toString(), event.nodeId2()
					.toString())).enluminerBis(state.isSynchronized());
			// arete.eluminerBis(state.isSynchronized());
			vue.rechercherSommet(event.nodeId1().toString()).enluminerBis(
					state.isSynchronized());
			vue.rechercherSommet(event.nodeId2().toString()).enluminerBis(
					state.isSynchronized());
			// BILEL
			this.agentsSimulationWindow.simulationPanel().repaint();
		} else {
	...
	public void handleLabelChangeEvt(SimulEvent se) throws InterruptedException {
