...
/**
 * Class that maps properties (key, value) entries into a double column table
 * model.
 */
public class AgentPropertyTableModel extends AbstractPropertyTableModel {
	...
	public Object getValueAt(int row, int col) {
		...
		case 1:
			return this.getTypeName(this.wb.getValue(this.keys.elementAt(row)));
		case 2:
			return this.wb.getValue(this.keys.elementAt(row));
		}
	...
	public void setValueAt(Object aValue, int row, int col) {
		...
		try {
			...
			if (obj instanceof String) {
				...
				this.wb.setValue(this.keys.elementAt(row), Integer
						.decode(value));
				...
				this.wb.setValue(this.keys.elementAt(row), Double
						.parseDouble(value));
			} else if (obj instanceof Float) {
				this.wb.setValue(this.keys.elementAt(row), Float
						.parseFloat(value));
			} else if (obj instanceof Long) {
				this.wb.setValue(this.keys.elementAt(row), Long
						.parseLong(value));
			} else if (obj instanceof Short) {
				this.wb.setValue(this.keys.elementAt(row), Short
						.parseShort(value));
			} else if (obj instanceof Boolean) {
				this.wb.setValue(this.keys.elementAt(row), Boolean
						.parseBoolean(value));
			}
		} catch (Exception e) {
