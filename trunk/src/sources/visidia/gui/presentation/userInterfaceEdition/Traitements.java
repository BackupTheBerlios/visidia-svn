...
/**
 * La classe Traitement ne peut etre instanciée, elle contient simplement
 * quelques fonctions de traitement des graphes de l'éditeur Editeur. C'est une
 * "boite a outils".
 */
public class Traitements {
		...
		while (v_enum.hasMoreElements()) {
			...
			if (f.type().equals("edge")) {
				ensemble.inserer(((AreteDessin) f).getArete().origine()
						.getSommetDessin());
				ensemble.inserer(((AreteDessin) f).getArete().destination()
						.getSommetDessin());
			}
		}
		return ensemble;
	...
	/**
	 * Cette méthode sélectionne le sous_graphe connexe auquel appartient le
	 * sommet passe en argument. Le calcul tient compte de l'orientation des
	 * arêtes.
	 * 
	 * @return une selection contenant les sommets et les arêtes du sous-graphe
	...
	public static SelectionDessin sousGrapheConnexeOriente(
		 ...
		 * while(sommets_initiaux.hasMoreElements()) { sommet_courant =
		 * (Sommet)sommets_initiaux.nextElement();
		 * selection_connexe.insererElement(sommet_courant);
		 * sommets_restants.push(sommet_courant); } // boucle principale: un
		 * tour de boucle pour chacun des sommets rencontres do { sommet_courant =
		 * (Sommet)sommets_restants.pop(); // on s'occupe des sommets
		 * successeurs Enumeration successeurs = sommet_courant.successeurs();
		 * while(successeurs.hasMoreElements()) { Sommet un_successeur =
		 * (Sommet)successeurs.nextElement(); // Si c'est la premiere fois qu'on
		 * recontre le sommets, on le met // dans la selection
		 * if(!selection_connexe.contient(un_successeur)) {
		 * selection_connexe.insererElement(un_successeur);
		 * sommets_restants.push(un_successeur); } } // on s'occupe des aretes
		 * sortantes Enumeration aretes_sortantes =
		 * sommet_courant.aretesSortantes();
	...
	/**
	 * Cette méthode sélectionne le sous_graphe connexe auquel appartient le
	 * sommet passé en argument. Le calcul ne tient pas compte de l'orientation
	 * des arêtes.
	 * 
	 * @return une sélection contenant les sommets et les arêtes du sous-graphe
	 *         connexe contenant le sommet.
	 */
	public static SelectionDessin sousGrapheConnexe(Enumeration v_enum) {
		 ...
		 * sommets_restants.push(sommet_courant); } // boucle principale: un
		 * tour de boucle pour chacun des sommets rencontres do { sommet_courant =
		 * (SommetDessin)sommets_restants.pop(); // on s'occupe des sommets
		 * voisins Enumeration voisins = sommet_courant.sommetsVoisins();
		 * while(voisins.hasMoreElements()) { SommetDessin un_voisin =
		 * (SommetDessin)voisins.nextElement(); // Si c'est la premiere fois
		 * qu'on recontre le sommets, on le met // dans la selection
		 * if(!selection_connexe.contient(un_voisin)) {
		 * selection_connexe.insererElement(un_voisin);
		 * sommets_restants.push(un_voisin); } } // on s'occupe des aretes
		 * incidentes Enumeration aretes_incidentes =
		 * sommet_courant.aretesIncidentes();
