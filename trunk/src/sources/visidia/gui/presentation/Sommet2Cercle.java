...
public class Sommet2Cercle extends SommetDessin{
	...
	if(tag.equals("agents")) {
	    ...
	    int stringSize = (int)(g.getFontMetrics().
				   ...
				   getWidth());
	    g.drawString(mesg,this.centreX()-stringSize/2 , this.centreY()+this.getCote()/4 );
	}	    
    }
	...
	g.fillOval(this.posx- _rayon/2-4,this.posy- _rayon/2-2,_rayon+8,_rayon+8);
	...
	g.drawOval(this.posx-_rayon/2-4,this.posy- _rayon/2 -2,_rayon+8,_rayon+8);
	...
	if((this.getVueGraphe()).afficherEtiquettes()){
	    ...
	    g.drawString((this.getEtiquette())+ " , "+this.getEtat(),x - _rayon /2 , y + _rayon + 8);
	...
	g.setColor((Color)((TableCouleurs.getTableCouleurs()).get(this.getEtat())));  
	...
	return (((x1 <= (this.posx - monCote)) && (y1 <= this.posy - monCote)) &&
		((x2 >= (this.posx + monCote )) && (y2 >= this.posy + monCote / 2)));
    }
	...
	Sommet2Cercle sommetRetour = new Sommet2Cercle(vueGraphe,this.posx,this.posy,new String(this.monEtiquette),le_clone);
