...
public abstract class AreteDessin extends FormeDessin {
	...
	public AreteDessin(SommetDessin origine, SommetDessin destination) {
		this(origine, destination, new Arete(
				origine.getVueGraphe().getGraphe(), origine.getSommet(),
				destination.getSommet()));
	}

	// PFA2003
	protected void dessinerLigne(Graphics g, float largeur, int x1, int y1,
			int x2, int y2) {
		...
		/*
		 * } else { float[] dash = {6.0f, 4.0f, 2.0f, 4.0f, 2.0f, 4.0f};
		 * BasicStroke bs = new BasicStroke(largeur, BasicStroke.CAP_BUTT,
		 * BasicStroke.JOIN_MITER, 10.0f, dash, 0.0f); ((Graphics2D)
		 * g).setStroke(bs); }
		 */
	...
	public void dessiner(Component c, Graphics g) {
		if (this.etatArete) {
			...
			this.dessinerLigne(g, 5.0f, this.origx, this.origy, this.destx,
					this.desty);
		} else {
			if (this.enlumineBis) {
				g.setColor(COULEUR_ENLUMINER_BIS);
			...
			g.setColor(this.enlumine ? this.couleur_fond : this.couleur_trait);
			this.dessinerLigne(g, 1.0f, this.origx, this.origy, this.destx,
					this.desty);
		}

		// if(isOriented){ à revoir
		if (false) {
			Shape arrowHead = ArrowHeadFactory.createSegmentArrowHead(
					new Point(this.origx, this.origy), new Point(this.destx,
							this.desty), 6, 10);
	...
	public void repositionner(SommetDessin origine, SommetDessin destination) {
		int x1 = origine.centreX(), x2 = destination.centreX();
		int y1 = origine.centreY(), y2 = destination.centreY();
		int dx = x2 - x1, dy = y2 - y1;
		float dist = (float) Math.sqrt((dx * dx + dy * dy));

		if (dist > 0) {
			float theta = (float) Math.atan2(dx, dy);
			float r1 = origine.rayon(theta), r2 = destination.rayon(theta
					+ ((theta > 0) ? (-(float) Math.PI) : (float) Math.PI));

			if ((r1 + r2) < dist) {
				float cos_theta = dx / dist, sin_theta = dy / dist;

				this.placerOrigine(x1 + (int) (r1 * cos_theta), y1
						+ (int) (r1 * sin_theta));
				this.placerDestination(x2 - (int) (r2 * cos_theta), y2
						- (int) (r2 * sin_theta));
				return;
			}
		}
	...
	private void repositionnerDestination(SommetDessin destination) {
		...
		if (dist > 0) {
			float theta = (float) Math.atan2(dx, dy);
			float r2 = destination.rayon(theta
					+ ((theta > 0) ? (-(float) Math.PI) : (float) Math.PI));
	...
	public boolean appartient(int x, int y) {
		double scalaire = (x - this.origx) * (this.destx - this.origx)
				+ (y - this.origy) * (this.desty - this.origy);
		double long_carre = Math.pow(this.destx - this.origx, 2)
				+ Math.pow(this.desty - this.origy, 2);

		if (scalaire > 0) {
			double distance_carre = ((Math.pow(x - this.origx, 2) + Math.pow(y
					- this.origy, 2))
	...
	// (x1, y1) are the top left side coordinates and (x2, y2) are the bottom
	// right side coordinates
	public boolean estDansRegion(int x1, int y1, int x2, int y2) {
		return ((x1 <= this.origx) && (y1 <= this.origy) && (x2 >= this.destx)
				&& (y2 >= this.desty) && (x1 <= this.destx)
				&& (y1 <= this.desty) && (x2 >= this.origx) && (y2 >= this.origy));
	}
	...
	public void changerDestination(SommetDessin nouvelleDestination) {
		((Arete) this.graphObject).changerDestination(nouvelleDestination
				.getSommet());
	...
	/**
	 * returns the graphic properties of the edge shape foreground color
	 * enluminated color
	 */
	public String graphicProperties() {
		return new String(this.forme() + "\t" + this.couleurTrait().getRed()
	...
	/**
	 * Returns a window 'BoiteFormeDessin' wich shows theproperties of the edge.
	 * Call the pach() and show() methods to display
	 * 
	...
	// Definition of the method 'cloner' which must be implemented in the
	// under-classes
	public abstract Object cloner(SommetDessin origine, SommetDessin destination);

	// method which copy all the variable from the AreteDessin given in
	// parameters
