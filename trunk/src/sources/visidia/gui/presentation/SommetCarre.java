...
public class SommetCarre extends SommetDessin {
	...
	public SommetCarre(VueGraphe vg, int posx, int posy, String etiquette,
			Sommet s) {
	...
	public void dessiner(Component c, Graphics g) {
		...
		g.drawRect(this.posx - cote_sur_2 - 1, this.posy - cote_sur_2 - 1,
				2 * cote_sur_2 + 2, 2 * cote_sur_2 + 2 + 2 * cote_sur_2 / 3);
		g.drawRect(this.posx - cote_sur_2 - 2, this.posy - cote_sur_2 - 2,
	...
	public Object cloner(VueGraphe vueGraphe) {
		Sommet le_clone = (Sommet) this.getSommet().cloner(
				vueGraphe.getGraphe());
		SommetCarre sommetRetour = new SommetCarre(vueGraphe, this.posx,
				this.posy, new String(this.monEtiquette), le_clone);
		sommetRetour.copyAllVariable(this);
		return sommetRetour;
	}

	// method which copy all the variable from the SommetCarre given in
	// parameters
