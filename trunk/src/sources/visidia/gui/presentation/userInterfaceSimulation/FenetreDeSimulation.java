...
public class FenetreDeSimulation extends Fenetre implements Serializable,
	...
	protected JMenuItem graph_open, algo_open, algo_open_vertices, graph_save,
			graph_save_as, file_quit, file_close, file_help, graph_select_all,
	...
	public FenetreDeSimulation(VueGraphe grapheVisu_edite, Color couleur_fond,
			int dim_x, int dim_y, File fichier_edit) {
	...
	protected void addMenu() {
		...
		this.item_nothing.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_N,
				ActionEvent.CTRL_MASK));
		...
		this.item_saveTrace.setAccelerator(KeyStroke.getKeyStroke(
		...
		this.item_replay.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_R,
				ActionEvent.CTRL_MASK));
		...
		this.item_chose.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_O,
				ActionEvent.CTRL_MASK));
	...
	protected void addToolBar() {
		...
		if (FenetreDeSimulation.threadCountFrame == null) {
			FenetreDeSimulation.threadCountFrame = new ThreadCountFrame(Thread
	...
	public void but_start() {
		...
		if (this.item_saveTrace.isSelected()) {
			...
			RecorderEvent recorderEvent = new RecorderEvent(this.evtPipeIn,
					this.evtPipeOut, this.writer);
			RecorderAck recorderAck = new RecorderAck(this.ackPipeIn,
					this.ackPipeOut, this.writer);
			...
			this.sim = new Simulator(Convertisseur.convertir(this.vueGraphe
					.getGraphe()), this.evtPipeIn, this.ackPipeOut,
					this.algoChoice);
		} else if (this.item_replay.isSelected()) {
			visidia.simulation.Reader reader = new visidia.simulation.Reader(
		 ...
		 * AlgoRule(rulesList)); else if(!algoChoice.verticesHaveAlgorithm()) {
		 * if (!item_replay.isSelected()) { JOptionPane.showMessageDialog(this,
		 * "you must enter an algorithm or rules ", "warning",
	...
	public void but_experimentation() {
		if ((this.vueGraphe.getGraphe().sommets().hasMoreElements())
			...
			JFrame frame = new ExperimentationFrame(this.vueGraphe,
					this.algoChoice);
			...
			JOptionPane
					.showMessageDialog(this, "Load a graph and an algorithm");
		}
	}
	...
	public void menuAlgo(JMenuItem mi) {
		...
		if (mi == this.algo_open) {
			if (this.simulationRules) {
				JOptionPane.showMessageDialog(this,
						"you had already entered rules", "warning",
						JOptionPane.WARNING_MESSAGE);
			}
		...
		if (mi == this.rules_open) {
			...
			if (this.simulationAlgo) {
				int res = JOptionPane
						.showConfirmDialog(
								this,
								"The algorithm has already been selected ;\n if "
										+ "you continue, you will not be able to apply new rules.\n "
										+ "Continue ? ", "Open new rule",
	...
	public void menuTrace(JMenuItem mi) {
		if (mi == this.item_replay) {
			...
			if (f != null) {
				...
				OpenGraph.open(this, this.fileSaveTrace);
				this.algoChoice = new AlgoChoice(this.vueGraphe.getGraphe()
						.ordre());
				this.replaceSelection(new SelectionDessin());
				this.simulationPanel.setPreferredSize(this.vueGraphe
						.donnerDimension());
				this.simulationPanel.revalidate();
				this.simulationPanel.scrollRectToVisible(new Rectangle(650,
						600, 0, 0));
				this.simulationPanel.repaint();
			}
		}
	...
	private void propertiesControl() {
		if (this.selection.estVide()) {
			...
			if (!Traitements.sommetDessin(this.selection.elements())
				...
				BoiteChangementCouleurArete boiteArete = new BoiteChangementCouleurArete(
						this, listeElements);
				boiteArete.show(this);
			} else if ((this.selection.nbElements() == 1)
					&& (firstElement.type().equals("vertex"))) {
				BoiteChangementEtatSommet boiteSommet = new BoiteChangementEtatSommet(
