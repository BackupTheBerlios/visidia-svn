...
public class ExperimentationFrame extends JFrame implements ActionListener {
		...
		if (this.fileChooser.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
			...
			try {
				...
				for (int i = 0; i < rowCount; i++) {
					String numString = this.tableModel.getValueAt(i, 0)
							.toString();
					...
					String mesgCountString = this.tableModel.getValueAt(i, 1)
							.toString();
					StringBuffer mesgCountStringBuff = new StringBuffer(
							mesgCountBlank);
					mesgCountStringBuff.replace(0, mesgCountString.length(),
							mesgCountString);
	...
	public void start() {
		try {
			this.sampleNumber = new Integer(this.testNumberTextField.getText())
					.intValue();
		...
		this.expThread = new ExperimentationThread(this.algoChoice, this.graph,
				this.sampleNumber);
	...
	class ExperimentationThread extends Thread {
		...
		public ExperimentationThread(AlgoChoice algoFactory,
				VueGraphe vueGraphe, int nbSample) {
		}
		...
		public void run() {
			for (int i = 0; i < ExperimentationFrame.this.sampleNumber; i++) {
				...
				this.simulator = new Simulator(
						Convertisseur.convertir(ExperimentationFrame.this.graph
								.getGraphe()), this.eventVQueue,
						this.ackVQueue, ExperimentationFrame.this.algoChoice);
				this.simulator.startSimulation();
				this.terminated = false;

				ExperimentationFrame.this.controlEnabling();
				ExperimentationFrame.this.status.setText("simulation '" + i
						+ "' is running...");
		...
		public void handleNodePropertyChangeEvt(SimulEvent se)
			...
			NodePropertyChangeAck npca = new NodePropertyChangeAck(se
					.eventNumber());
	 ...
	 * ExperimentationFrame(); frame.addWindowListener(new
...
class ExperimentationResultTableModel extends AbstractTableModel {
	...
	public Object getValueAt(int row, int col) {
		...
		case 1:
			return this.lineVector.get(row);
		}
		throw new IllegalArgumentException();
	}
