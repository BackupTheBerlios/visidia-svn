...
public class AgentsSimulationWindow extends Fenetre implements Serializable,
    ...
    /**
         * Bouton pour passer en mode suppression : aretes et sommets
         */
    protected JButton but_suppression;
    ...
    private Hashtable<String, Object> defaultProperties; // To initialize
	...
	super.setSize(Fenetre.DIM_X_PAR_DEFAUT, Fenetre.DIM_Y_PAR_DEFAUT);
	...
	this.but_default.setAlignmentY(Component.CENTER_ALIGNMENT);
	...
	this.but_agents.setAlignmentY(Component.CENTER_ALIGNMENT);
	...
	this.but_suppression.setToolTipText("Kill vertex/edge");
	...
	this.toolBar.add(this.but_suppression);
	...
	this.toolBar.addSeparator();
	if (AgentsSimulationWindow.threadCountFrame == null) {
	    AgentsSimulationWindow.threadCountFrame = new ThreadCountFrame(
	...
	if (this.selection.estVide())
	    JOptionPane.showMessageDialog(this,
		    "You should select the vertex you want to switch on!",
		    "Error", JOptionPane.WARNING_MESSAGE);
    ...
    /**
         * Swithes off the selected vertex. You have to select the vertex before
         * swithing it off. The vertex you select should not be a vertex alraedy
         * swith off.
         * 
         */
	...
	if (b == this.but_start) {
	    ...
	    AgentsSimulationWindow.threadCountFrame.pack();
	    AgentsSimulationWindow.threadCountFrame.setVisible(true);
	  ...
	  else if (b == this.but_suppression) {
		    this.but_suppression();
	}
    ...
    /** ***************************************************************** */
    /***********************************************************************
         * Returns the identity of a selected vertex contained in
         * this.selection.elements() : this method is only used in menulAlgo() /
         **********************************************************************/
    private Integer getId(Enumeration e) {
	return Integer.decode(((SommetDessin) e.nextElement()).getEtiquette());
    }

    /** ********************************************************* */
    /***********************************************************************
         * Returns the first class name in the hashtable "agentsTable" This
         * method is only used in menulAlgo() /
         **********************************************************************/
    private String getClassName(Hashtable agentsTable) {
	return (String) ((ArrayList) agentsTable.get(this.agentsTable.keys()
		.nextElement())).get(0);
    }
	...
	if (mi == this.algo_open) {
		...
		if (!this.but_stop.isEnabled()) {
		    ...
		    Integer id = getId(this.selection.elements());
		    ...
		    this.sim.createAgentDuringExecution(className, id,
	...
	/** ************** */
	/*
         * if (mi == this.algo_killAgent) { if (this.selection.estVide()) {
         * JOptionPane.showMessageDialog(this, "You must select at least" + "
         * one vertex or one agent !", "Warning", JOptionPane.WARNING_MESSAGE);
         * return; } }
         */
	...
	AgentsSimulationWindow.visuAlgorithmMess = b;
    }

    public static void setVisuSynchrMess(boolean b) {
	AgentsSimulationWindow.visuSynchrMess = b;
    }
