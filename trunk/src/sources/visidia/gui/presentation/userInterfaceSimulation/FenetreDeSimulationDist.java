...
public class FenetreDeSimulationDist extends Fenetre implements Serializable,
		ActionListener, ItemListener, WindowListener, ChangeListener {
	...
	protected JButton but_start, but_pause, but_save, but_stop, but_help,
			but_experimentation, but_threadCount, but_information_distribue;
	...
	protected JMenuItem graph_open, algo_open, algo_open_vertices, graph_save,
			graph_save_as, file_quit, file_close, file_help, config_reseaux,
			config_reseaux_file, config_registration, config_help,
			config_registry, experiment_complet, experiment_begin;
	...
	public FenetreDeSimulationDist(VueGraphe grapheVisu_edite,
		...
		try {
			this.registry = LocateRegistry.createRegistry((new Integer(
					this.rmiRegistryPort)).intValue());
		} catch (RemoteException re) {
			try {
				this.registry = LocateRegistry.getRegistry((new Integer(
						this.rmiRegistryPort)).intValue());
			} catch (Exception e) {
				bool = false;
				JOptionPane.showMessageDialog(this,
						"Cannot initialize RMI Registry : \n" + e.toString(),
		...
		try {
			...
			JOptionPane.showMessageDialog(this,
					"The Visidia Registration is not running" + e, "Error",
	...
	protected void addMenu() {
		...
		this.item_visualization = new JCheckBoxMenuItem("Visualize Messages",
				true);
	...
	public void action_toolbar(JButton b) {
		if (b == this.but_start) {
			...
			if (this.networkParam == null) {
				...
				JOptionPane.showMessageDialog(this,
						"you must configure the net for each \n"
			...
			if (DistributedAlgoSimulator.estStandalone()) {
				try {
					this.sim_Rmi = new Simulator_Rmi(Convertisseur
							.convertir(this.vueGraphe.getGraphe()),
							this.evtPipeOut, this.ackPipeOut,
							this.simulatorHost, this.simulatorUrl,
							this.rmiRegistryPort);
					Naming.bind("rmi://:" + this.rmiRegistryPort + "/"
							+ this.simulatorUrl, this.sim_Rmi);
				} catch (Exception e) {
					System.out
							.println("An error has aquired when creating and binding the\n"
									+ "Simulator to the localhost : <"
									+ this.simulatorHost
									+ ","
									+ this.rmiRegistryPort
									+ ","
									+ this.simulatorUrl + ">\n" + e);
					JOptionPane.showMessageDialog(this,
							"An error has aquired when creating and binding the\n"
									+ "Simulator to the localhost : <"
									+ this.simulatorHost + ","
									+ this.rmiRegistryPort + ","
									+ this.simulatorUrl + ">\n" + e, "Error",
			...
			try {
				...
				System.out
						.println("An Error has aquired when starting, initializing and runnig the nodes \n"
								+ e);
				JOptionPane.showMessageDialog(this,
						"An Error has aquired when starting, initializing and runnig the nodes \n"
			...
			/*
			 * a revoir try { sim_Rmi.abortSimulation();
			 * Naming.unbind("rmi://:"+rmiRegistryPort+"/"+simulatorUrl); }
			 * catch (Exception e) { System.out.println("Erreur dans le unbind :
			 * "+e); }
			 */
			...
			try {
				System.out
						.println("the number of messages exchanged at this stage is "
								+ this.sim_Rmi.getMessageNumber());
				// JOptionPane.showMessageDialog(this, "the number of messages
				// exchanged at this stage is "+sim_Rmi.getMessageNumber());
			} catch (Exception e) {
			...
			try {
				...
				Naming.unbind("rmi://:" + this.rmiRegistryPort + "/"
						+ this.simulatorUrl);
			...
			try {
				...
				// RemoteObjectBoite rob = new RemoteObjectBoite(this,"Location
				// of Nodes",table,configHosts);
				RemoteObjectBoite rob = new RemoteObjectBoite(this,
						"Location of Nodes", table);
				rob.show(this);
			} catch (Exception e) {
			}
		}
	...
	public void menuConfig(JMenuItem mi) {
		if (mi == this.config_help) {
			OpenHelpDist openHelpDist = new OpenHelpDist(
					"Help on the configuration");
			openHelpDist.show();
		} else if (mi == this.config_registry) {
			if (DistributedAlgoSimulator.estStandalone()) {
				BoiteRegistry boiteRegistry = new BoiteRegistry(this,
						"Registry configuration");
				boiteRegistry.show(this);
			}
		} else if (mi == this.config_reseaux) {
			BoiteDistribue boiteDistribue = new BoiteDistribue(this,
					this.vueGraphe.cloner(), "configuration du reseaux");
			...
			if (DistributedAlgoSimulator.estStandalone()) {
				try {
					if (!this.rmiRegistryPort.equals("1099")) {
						this.registry.unbind("rmi://:" + this.rmiRegistryPort
								+ "/Registry");
						this.registry = null;
						this.registry = LocateRegistry
								.createRegistry((new Integer(
										this.rmiRegistryPort)).intValue());
						this.vr.init("Registry", this.registry);
						this.vr.showLocalNodes(this.vueGraphe.getGraphe()
								.ordre());
					} else {
						this.vr.showLocalNodes(this.vueGraphe.getGraphe()
								.ordre());
	...
	public void menuExperiment(JMenuItem mi) {
		if (mi == this.experiment_complet) {
			BoiteExperimentComplet bec = new BoiteExperimentComplet(this,
					"size");
			bec.show(this);
	...
	public void itemStateChanged(ItemEvent evt) {
		if ((JCheckBoxMenuItem) evt.getSource() == this.item_visualization) {
			Enumeration enumerationSommets = this.vueGraphe.getGraphe()
					.sommets();
			Sommet unSommet;
			boolean bool;
			String boolString;
			...
			while (enumerationSommets.hasMoreElements()) {
				...
				unSommet.getSommetDessin()
						.setValue("Draw Messages", boolString);
			}
			if (this.sim_Rmi != null) {
				try {
					...
					System.out
							.println("Erreur au niveau Fenetre de Simulation");
				}
			}
		}
	}
	...
	private void propertiesControl() {
		if (this.selection.estVide())
			...
			if (!Traitements.sommetDessin(this.selection.elements())
				...
				BoiteChangementEtatSommetDist boiteSommet = new BoiteChangementEtatSommetDist(
	...
	public void nodeStateChanged(int nodeId, Hashtable properties) {
		try {
			...
			System.out
					.println("Erreur lors de la modification des proprietes dans FenetreDeSimulation : "
							+ e);
		}
	}
