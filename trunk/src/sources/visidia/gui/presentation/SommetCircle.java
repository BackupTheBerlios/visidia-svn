...
public class SommetCircle extends SommetDessin {
	...
	public SommetCircle(VueGraphe vg, int posx, int posy, String etiquette,
			Sommet s) {
	...
	public void dessiner(Component c, Graphics g) {
		...
		g.drawOval(this.posx - cote_sur_2 - 2, this.posy - cote_sur_2 - 2,
				2 * cote_sur_2 + 4, 2 * cote_sur_2 + 4);
		g.drawOval(this.posx - cote_sur_2, this.posy - cote_sur_2,
		...
		g.fillOval(this.posx - cote_sur_2, this.posy - cote_sur_2,
				2 * cote_sur_2, 2 * cote_sur_2);
	 ...
	 * sommet. coordonnees, forme, cote, couleur du trait, couleur
	 * d'enlumination.
	 */
	...
	public Object cloner(VueGraphe vueGraphe) {
		Sommet le_clone = (Sommet) this.getSommet().cloner(
				vueGraphe.getGraphe());
		SommetCircle sommetRetour = new SommetCircle(vueGraphe, this.posx,
				this.posy, new String(this.monEtiquette), le_clone);
		sommetRetour.copyAllVariable(this);
		return sommetRetour;
	}

	// method which copy all the variable from the SommetCircle given in
	// parameters
