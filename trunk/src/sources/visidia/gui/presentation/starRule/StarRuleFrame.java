...
public class StarRuleFrame extends JFrame implements RuleTabbedPaneControl {
	...
	public StarRuleFrame(JFrame parent, final ApplyStarRulesSystem applyRules) {
		...
		this.helpDialog = new HelpDialog(this.finalThis,
				"Insert the rules description");
		...
		butApply.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int res = JOptionPane.showConfirmDialog(
						StarRuleFrame.this.finalThis,
						"Apply the rules to the simulation frame ?",
						"Apply the rules", JOptionPane.YES_NO_OPTION);
				if (res == JOptionPane.YES_OPTION) {
					applyRules.applyStarRulesSystem(StarRuleFrame.this
							.getRelabelingSystem());
				}
			}
		});
	...
	private void buildFileMenu(JMenu menuFile) {
		...
		fileNew.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int res = JOptionPane.showConfirmDialog(
						StarRuleFrame.this.finalThis, "Begin new system ?",
						"Begin new system", JOptionPane.YES_NO_OPTION);
				if (res == JOptionPane.YES_OPTION) {
		...
		fileClose.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int res = JOptionPane.showConfirmDialog(
						StarRuleFrame.this.finalThis, "Close the frame ?",
						"Close the frame", JOptionPane.YES_NO_OPTION);
				if (res == JOptionPane.YES_OPTION) {
					StarRuleFrame.this.finalThis.setVisible(false);
					StarRuleFrame.this.fileName = null;
					fileSave.setEnabled(false);
		...
		fileSaveAs.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				int returnVal = chooser
						.showSaveDialog(StarRuleFrame.this.finalThis);
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (returnVal == JFileChooser.APPROVE_OPTION) {
					try {
						...
						RelabelingSystem rSys = (RelabelingSystem) p
								.readObject();
	...
	public RelabelingSystem getRelabelingSystem() {
		...
		int synType = (this.synRdv.isSelected() ? SynCT.RDV : (this.synLC1
				.isSelected() ? SynCT.LC1
				: (this.synLC2.isSelected() ? SynCT.LC2 : -1)));
		RSOptions opt = new RSOptions(synType, this.optionTermination
				.isSelected());
		rSys.setOptions(opt);
...
class RulePane extends JPanel implements ContexTabbedPaneControl {
	...
	public RulePane(RuleTabbedPaneControl tabbedPaneControl, Rule rule) {
		...
		if (rule == null) { // There is no rule
			this.context.addTab("Context 1", new BuildContextPane(this, true,
					null));
			...
			if (rule.forbContexts().size() == 0) {
				...
				this.context.addTab("Context 1", new BuildContextPane(this,
						true, null));
			} else {
				for (Iterator it = rule.forbContexts().iterator(); it.hasNext();) {
					Star s = (Star) it.next();
					this.context.addTab("",
							new BuildContextPane(this, false, s));
				}
			}
			int t = rule.getType();
			if (t == SynCT.GENERIC)
		...
		comboTermination.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				RulePane.this.terminationType = comboTermination
						.getSelectedIndex();
			}
		});
	...
	public void addNewContext() {
		...
		this.context.addTab("",
				new BuildContextPane(this, false, centerContext));
		this.renameContext();
	...
	public void insertContext() {
		...
		this.context.insertTab("", null,
				new BuildContextPane(this, false, null), null, pos);
		this.renameContext();
	}
}
...
class ConvertStarVueGraph {
	...
	/**
	 * The VueGraphe must already contain sommetC. Adds to vg the data and the
	 * other vertexes stored into star. The graphical position of the elements
	 * must be reorganized.
	 */
	static public void star2StarVueGraphe(Star star, VueGraphe vg,
...
class BuildRulePane extends JPanel {
	...
	JMenuItem switchLeftMenuItem, switchRightMenuItem,
			reorganizeVertexMenuItem;
	...
	public BuildRulePane(final RuleTabbedPaneControl tabbedPaneControl,
		...
		this.sommetCLeft = this.vgLeft.creerSommet(this.centerLeft.x,
				this.centerLeft.y);
		this.sommetCRight = this.vgRight.creerSommet(this.centerRight.x,
				this.centerRight.y);

		if ((sLeft != null) && (sRight != null)) {
			ConvertStarVueGraph.star2StarVueGraphe(sLeft, this.vgLeft,
					this.sommetCLeft);
			ConvertStarVueGraph.star2StarVueGraphe(sRight, this.vgRight,
					this.sommetCRight);
		}

		this.dsvp = new DuoStarVisuPanel(this, this.vgLeft, this.vgRight,
				this.centerLeft, this.centerRight, StarData.ray * 2
						+ StarData.rule_center, StarData.ray, this.isSimpleRule);
		...
		this.simpleRuleMenuItem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if ((BuildRulePane.this.vgLeft.nbObjets() > 3)
						|| (BuildRulePane.this.vgRight.nbObjets() > 3)) {
					BuildRulePane.this.simpleRuleMenuItem
							.setSelected(BuildRulePane.this.isSimpleRule);
					return;
				}
				BuildRulePane.this.isSimpleRule = BuildRulePane.this.simpleRuleMenuItem
						.isSelected();
				BuildRulePane.this.setCenterPosition();
				BuildRulePane.this.sommetCLeft.placer(
						BuildRulePane.this.centerLeft.x,
						BuildRulePane.this.centerLeft.y);
				BuildRulePane.this.sommetCRight.placer(
						BuildRulePane.this.centerRight.x,
						BuildRulePane.this.centerRight.y);
	...
	private void setCenterPosition() {
		if (this.isSimpleRule) {
			...
			this.centerRight = new Point(this.centerLeft.x + StarData.ray * 2
					+ StarData.rule_center, StarData.ray + StarData.rule_top);
		} else {
	...
	private void maybeShowPopup(MouseEvent e) {
		if (e.isPopupTrigger()) {
			this.switchRightMenuItem.setEnabled(this.tabbedPaneControl
					.canSwitchRight());
			this.switchLeftMenuItem.setEnabled(this.tabbedPaneControl
					.canSwitchLeft());
	...
	public Dimension getPreferredSize() {
		return new Dimension(StarData.rule_left + StarData.ray * 2
				+ StarData.rule_center + StarData.ray * 2 + StarData.rule_left,
				StarData.rule_top + StarData.ray * 2 + StarData.rule_bottom);
	}

	public Star getLeftStar() {
		return ConvertStarVueGraph.starVueGraphe2Star(this.vgLeft,
				this.sommetCLeft);
	}

	public Star getRightStar() {
		return ConvertStarVueGraph.starVueGraphe2Star(this.vgRight,
				this.sommetCRight);
	}
	...
	public void paintComponent(Graphics g) {
		...
		if (!this.isSimpleRule) {
			...
			g.drawLine(this.centerRight.x, this.centerRight.y - StarData.ray
					- 2, this.centerRight.x, this.centerRight.y - StarData.ray
					+ 2);
		}
		// Center arrow
		((Graphics2D) g).setStroke(new BasicStroke(3.0f));
		g.drawLine(StarData.arrow_x1, this.centerLeft.y, StarData.arrow_x2,
				this.centerLeft.y);

		g.drawLine(StarData.arrow_x2, this.centerLeft.y, StarData.arrow_x2 - 5,
				this.centerLeft.y - 5);
		...
		if (!this.isSimpleRule) {
			...
			g.drawArc(this.centerLeft.x - StarData.ray, this.centerLeft.y
					- StarData.ray, StarData.ray * 2, StarData.ray * 2, 0, 360);
			g.drawArc(this.centerRight.x - StarData.ray, this.centerRight.y
					- StarData.ray, StarData.ray * 2, StarData.ray * 2, 0, 360);
		} else {
			...
			g.drawLine(this.centerRight.x + StarData.ray,
					this.centerRight.y - 5, this.centerRight.x + StarData.ray,
					this.centerRight.y + 5);
			g.drawLine(this.centerRight.x + StarData.ray - 5,
					this.centerRight.y, this.centerRight.x + StarData.ray + 5,
					this.centerRight.y);
		}
...
/**
 * A BuildContextPane is a panel where a context is composed. A popup menu
 * proposes various actions (Add/Delete/Insert context) implemented by RulePane.
 */
class BuildContextPane extends JPanel {
	...
	public BuildContextPane(final ContexTabbedPaneControl tabbedPaneControl,
		...
		if (!empty) {
			this.svp = new StarVisuPanel(this.vg, StarData.ray, this.center,
					this, false);
			if (star != null) {
		...
		JMenuItem reorganizeVertexMenuItem = new JMenuItem(
				"Reorganize vertexes");
	...
	public Dimension getPreferredSize() {
		return new Dimension(StarData.ctxt_left + StarData.ray * 2
				+ StarData.ctxt_right, StarData.ctxt_top + StarData.ray * 2
	...
	public Star getStar() {
		if (this.isEmpty()) {
			...
			return ConvertStarVueGraph
					.starVueGraphe2Star(this.vg, this.sommetC);
		}
	}
	...
	public void paintComponent(Graphics g) {
		...
		g.setColor(Color.black);
		if (this.empty) {
			g.drawString("No context", this.center.x - StarData.ray / 2,
					this.center.y);
		} else {
			...
			Stroke tmp = ((Graphics2D) g).getStroke();
			((Graphics2D) g).setStroke(this.dashS);
			g.drawArc(this.center.x - StarData.ray, this.center.y
					- StarData.ray, StarData.ray * 2, StarData.ray * 2, 0, 360);
			((Graphics2D) g).setStroke(tmp);
