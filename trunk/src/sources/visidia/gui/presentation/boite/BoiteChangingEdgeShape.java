 ...
 * 
 */
public class BoiteChangingEdgeShape extends BoiteChangingShape implements
	...
	// Construsteurs
	/**
	 * Cree une nouvelle boite pour afficher les caractéristiques de "une_arete"
	 */
	...
	/**
	 * Crée une nouvelle boite, centrée sur "parent" pour afficher les
	 * caractéristiques de "une_arete". La boite sera appelée "titre" Suivant la
			...
			for (String element : listOfFiles)
				if (this.accept(element)) { // we keep only .class files
					...
					if (fabriqueArete.isAssignableFrom(factFile)) {
						FabriqueArete ourFactory = (FabriqueArete) factFile
								.newInstance();
	...
	/**
	 * Cette methode est appelée quand l'utilisateur appuie sur le bouton Ok
	 */
	public void buttonOk() {
		if (this.chgtForme != -1) {
			try {
				this.vueGraphe.changerFormeArete(
						(FabriqueArete) this.edgesFactories
