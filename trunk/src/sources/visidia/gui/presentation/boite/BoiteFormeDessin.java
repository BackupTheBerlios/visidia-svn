 ...
 * d'une FormeDessin. Les sous-classes de cette classe la raffinent en y
 * ajoutant de nouveaux elements.
 */
public class BoiteFormeDessin implements ActionListener {
	...
	/**
	 * La partie de fenetre qui permet d'afficher les caracteristiques de
	 * l'FormeDessin.
	 */
	...
	/**
	 * Ce booleen vaut VRAI si les caracteristiques de la FormeDessin peuvent
	 * etre modifiees, et FAUX sinon.
	 */
	 ...
	 * caracteristiques de "forme". Suivant la valeur de "est_editable", les
	 * caracteristiques de la FormeDessin seront modifiables.
	 */
	public BoiteFormeDessin(JFrame parent, FormeDessin forme, String titre,
		...
		BoxLayout caracteristicsLayout = new BoxLayout(this.caracteristicsPane,
				BoxLayout.Y_AXIS);
		this.caracteristicsPane.setLayout(caracteristicsLayout);

		this.caracteristicsPane.setBorder(BorderFactory.createTitledBorder(
				BorderFactory.createEtchedBorder(), "Caracteristics"));

		this.ligne_non_editable(this.caracteristicsPane, "Object type :", forme
				.type());

		this.couleur_trait = new LigneChoixCouleur(this,
				"Line color (R,G,B) : ", (forme).couleurTrait().getRed(),
				(forme).couleurTrait().getGreen(), (forme).couleurTrait()
						.getBlue(), est_editable);
		this.caracteristicsPane.add(this.couleur_trait.panel());

		this.dialog.getContentPane().setLayout(new BorderLayout());
		this.dialog.getContentPane().add(this.caracteristicsPane,
				BorderLayout.NORTH);
	...
	/***************************************************************************
	 * Ajoute un bouton nomme "label" au panel "pane" public JButton
	 * addButton(JPanel pane, String label) { JPanel tmp = new JPanel(new
	 * FlowLayout()); JButton button = new JButton(label); tmp.add(button);
	 * button.setSize(button.getMinimumSize()); pane.add(tmp);
	 * pane.add(Box.createRigidArea(new Dimension(0, 5))); return button; }
	 * 
	 * 
	 * /** Ajoute une ligne au panel "pane", qui permet d'afficher le texte
	 * "label" et la valeur "data". La valeur "data" ne sera pas modifiable.
	 */
	 ...
	 * 
	 * @param pane
	 *            le panel ou on ajoute ces elements
	 * @param label
	 *            le texte affiche dans le label.
	 * @param initialValue
	 *            la valeur initiale affichee dans le champ de saisie.
	 * @param editable
	 *            si ce booleen vaut VRAI, le champ de saisie est modifiable
	 * @return Le champ de saisie.
	 */
	public JTextField ligne_editable(JPanel pane, String label,
	 ...
	 *            le texte affiche dans le label.
	 * @param choices[]
	 *            un tableau qui contient les valeurs a inclure dans le liste de
	 *            choix.
	 * @param initial_choice
	 *            la valeur initiale affichee par la liste de choix.
	 * @param editable
	 *            si ce booleen vaut VRAI, la selection de la liste de choix est
	 *            modifiable
	 * @return La liste de choix.
	 */
	public JComboBox ligne_choix(JPanel pane, String label, Vector choices,
			boolean editable, Object initial_choice) {
		JPanel tmp = new JPanel(new GridLayout(1, 2));
		tmp.add(new JLabel(label));
	...
	/**
	 * Retourne VRAI si une des caracteristiques de l'FormeDessin a ete
	 * modifiee, et FAUX sinon.
	 */
	...
	public void buttonOk() {
		Color trait;
		try {
			trait = new Color(this.couleur_trait.getRed(), this.couleur_trait
					.getGreen(), this.couleur_trait.getBlue());
		} catch (NumberFormatException exception) {
	...
	/**
	 * Active ou desactive le bouton de choix de couleur, suivant la valeur du
	 * booleen passe en argument.
	 */
