...
public class BoiteDistribue implements ActionListener {
	...
	// Constructeurs
	public BoiteDistribue(FenetreDeSimulationDist parent, VueGraphe vueGraphe,
			String titre) {
		...
		this.sommets = BoiteDistribue.tri(vueGraphe.getGraphe().sommets(),
				this.nbrSommets);
	...
	public LocalNodeTable getParameter() {
		...
		for (int i = 4; i <= 3 * this.nbrSommets + 1; i++) {
			...
			if ((component instanceof JTextField)
					&& (componentUrl instanceof JTextField)) {
				try {
					...
					if (url.equals("")) {
						...
						if (lnt.containsHost(server)) {
							...
							// si ! ( il existe d'autres noeud assigne a la
							// machine sans Noeud locaux)
						...
						// On regarde dans les noeuds sauvegarde sans noeud
						// local
						// si on trouve des noeuds assignee a la machine du
						// noeud courant
						// on les rajoutent
						if (tmp.containsKey(server)) {
							Vector nodesWithoutLocalNode = (Vector) tmp
									.remove(server);
							lnt
									.addLocalNode(server, url,
											nodesWithoutLocalNode);
						}
					}
					j++;
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == this.buttonOk) {
			this.parent.setNetworkParam(this.getParameter(), this
					.getSimulatorHost(), this.getSimulatorUrl());
		...
		while (v_enum.hasMoreElements()) {
			...
			if (triSommet.isEmpty()) {
				...
				for (int i = 0; i < triSommet.size(); i++) {
					if (bool) {
						Integer tmp = new Integer(((Sommet) triSommet
								.elementAt(i)).getSommetDessin().getEtiquette());
