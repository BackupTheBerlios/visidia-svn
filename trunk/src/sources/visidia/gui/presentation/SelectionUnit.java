...
public class SelectionUnit extends MouseAdapter implements MouseMotionListener {
	...
	public void mouseReleased(MouseEvent evt) {
		...
		if (this.carre_selection) {
			...
			/*
			 * Selection des objets dans la zone rectangulaire : c'est un
			 * nouveau undoGroup.
			 */
			Enumeration e = this.recoverableObject.objetsDansRegion(
	...
	private void appuiShiftBoutonDroit(int x, int y) {
		try {
			...
			this.updateData();
			FormeDessin objet_sous_souris = this.recoverableObject.en_dessous(
					x, y);
			...
			if (this.selection.contient(objet_sous_souris)) {
				...
				if (this.undoInfo != null) {
					...
					this.undoInfo.addInfo(new DeselectFormeDessin(
							this.selection, objet_sous_souris));
				}
	...
	/**
	 * Must be called before using undoInfo, selectionDessin or
	 * recoverableObject
	 */
