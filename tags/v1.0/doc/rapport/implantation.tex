...
est  entièrement définie  et implantée  dans  la classe  Agent qui  se
trouve dans  le paquetage visidia.simulation.agents.   Les méthodes de
cette  classe sont  basées sur  des appels  aux méthodes  publiques du
...
contient principalement deux méthodes publiques :
\begin{itemize}
...
d'agents  héritant  d'une  de   ces  deux  classes  et  exécutant  des
...
paquetage visidia.tools.agents et qui  permet à l'agent de stocker des
...
été ajouté on n'aura pas alloué de l'espace inutilement.\\

\subsubsection{Mode de déplacement des agents}
Outres ces  structures, les  agents en contiennent  une autre  de type
AgentMover. Cette  structure définie le type de  déplacement de chaque
...
abstraite  et  est   à  redéfinir  pour  avoir  un   nouveau  type  de
...
fait sans  lui.  Il est défini  dans la classe  AgentSimulator dans le
paquetage   visidia.simulation.agents.   En   ce   qui  concerne   les
structures de données  utilisées, ce sont aussi des  tables de hachage
...
actions des agents sur le graphe avec lequel il a un lien direct grâce
à  une  variable d'instance.   C'est  aussi lui  qui  a  en charge  la
communication avec l'interface  graphique.  Le fonctionnement de cette
communication  est analogue à  celui du  simulateur déjà  existant. Le
simulateur informe  l'interface graphique d'un évènement  en créant un
objet de type approprié et  le place dans la file d'évènements.  Après
récupération   de  l´évènement,   l'interface  provoque   les  actions
correspondantes.   Les évènements  envoyés par  le simulateur  sont de
plusieurs types :\\
\begin{itemize}
...
de différents types correspondant  aux informations sur l'exécution de
...
\subsubsection{Structure utilisée}
...
et de  leur occurrences.  L'accès aux éléments  de cette  structure va
...
résultats  obtenus ne  soient pas  erronés afin  de ne  pas  créer des
dysfonctionnements  au sein du  programme.  Cette  gestion se  fait de
façon  simple en  Java  en utilisant  le  mot-clé $synchronized$.   Ce
mot-clé permet d'acquérir  un verrou sur un objet  quelconque pour une
durée limitée à l'exécution d'un simple bloc.

De plus,  les agents peuvent  demander de bloquer les  propriétés d'un
sommet lors de  l'exécution. Le blocage et le  déblocage se font grâce
aux  méthodes $lockVertexProperties$ et  $unlockVertexProperties$. Ces
méthodes utilisent le même mot-clé afin de gérer la concurrence.

\section{GUI}
Cette partie de  l'application est la plus importante  du point de vue
de l'utilisateur  car c'est  grâce à elle  qu'il va  pouvoir interagir
avec \visidia.

Le première  étape d'implantation ce cette  partie a été  de créer une
...
certaines de ses fonctionnalités lui sont spécifiques.

\subsection{Placement des agents}
Le placements des agents sur le  graphe peut se faire de deux manières
:\\
\begin{itemize} 
\item soit  de la manière traditionnelle en  sélectionnant les sommets
et les agents à placer.
\item soit en choisissant un type de placement particulier.
\end{itemize}
\sauteligne

Ce  type  de  placement est  en  fait  géré  par la  classe  abstraite
AgentChooser.  La méthode  abstraite $chooseForVertex$ est à redéfinir
afin  de choisir  le  comportement suivant  lequel  seront placer  les
agents. Il  est à noter  que les classes  dérivées vont se  charger de
...
fonctionnent  donc  de  la  même  façon.  Les  sommets  ont  donc  des
...
L'implantation  des  statistiques a  été  développée  dans la  section
...
que la simulation sera lancée et arrêtée. 

Cette fenêtre  provoquera la création  d'un thread qui se  chargera de
